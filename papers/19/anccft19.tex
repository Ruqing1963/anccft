% Algorithmic non-commutative class field theory, XIX
% Build: pdflatex anccft19 (x3)
...
\title[Algorithmic non-commutative class field theory, XIX]
{Algorithmic non-commutative class field theory, XIX:\\
the odd cyclic class that does not exist,\\ and what the exceptional zero sees instead}

\author{Ruqing Chen}
\address{GUT Geoservice Inc., Rivi\`ere-Beaudette, Qu\'ebec, Canada}
\email{ruqing@hotmail.com}
\date{19 September 2026}
\subjclass[2020]{Primary 19K56, 46L87; Secondary 05C50, 58B34, 11F72}
\keywords{cyclic cohomology, odd index pairing, crossed product, Floquet transform,
exceptional zero, $L$-invariant, spanning trees}
...
\section{The proposed algebra is the compacts}\label{sec:crossed}
...
\begin{proposition}\label{prop:compacts}
...
\section{The Floquet algebra, and what its pairing sees}\label{sec:floquet}
...
The natural element of it is the voltage pencil $D(t)=(q+1)I-A(t)$ of [IV, Lem.~1.1],
whose Floquet determinant is $\Theta$.

\begin{proposition}\label{prop:notinv}
$D$ is not invertible in $M_n(C(S^1))$: $\det D(1)=0$, since $D(1)$ is the ordinary
Laplacian of $Y$. The zero has order exactly two, and
$\tfrac12\Theta''(0)=-\kappa_0\|h_\alpha\|^2$ [IX, Thm.~1.2]. Hence the class $[D]\in K_1$
does not exist.
\end{proposition}

\begin{corollary}[What the pairing does see]\label{cor:winding}
...
$24$, $75$ and $175$ (Table~\ref{tab:data}). The contrast is the whole point: an index
...
\begin{remark}[Where the integrality comes from]\label{rem:integrality}
The task specification asked why the index should ``absorb'' $\kappa_0=\det G$. It does
not, and no index is involved: by [XIV, Thm.~7.1], $\|h_\alpha\|^2=a_c^{\mathsf T}G^{-1}a_c$
...
\section{The obstruction}\label{sec:obstruction}
...
\begin{corollary}\label{cor:applies}
The theorem applies to $M_n(C(S^1))$, where $K_1\cong\Z$, and to the shadow algebra
$\mathcal O_{B(Y)}$ of [VI], where $K_1\cong\Z$ by [VI, Thm.~2.1]. Both candidate
...
Theorem~\ref{thm:obstruction} is not a proof that no odd cyclic class anywhere computes
$\cL$; it rules out the linear constructions in rank-one $K_1$. Two escapes remain, and we
state them as the honest residue of [XIV, Rem.~9.1(a)]. \emph{Either} one finds an algebra
...
threshold at which Theorem~\ref{thm:obstruction} stops applying, and the smallest rank at
...
key & check & mode\\
\midrule
(A) & Proposition~\ref{prop:compacts}: free deck action, $n$ orbits & exact\\
(P) & the cleared pencil: $[T^0]=[T^1]=0$, $[T^2]=-\kappa_0\|h_\alpha\|^2$ & exact\\
(W) & Corollary~\ref{cor:winding}: the winding number is $2$ for every $\alpha$ & numeric\\
(I) & Remark~\ref{rem:integrality}: $\kappa_0\|h_\alpha\|^2=a_c^{\mathsf T}\operatorname{adj}(G)a_c\in\Z$ & exact\\
...
\emph{Proved.} Proposition~\ref{prop:compacts}, Proposition~\ref{prop:notinv} (given
[IX, Thm.~1.2]), Corollary~\ref{cor:winding}, Theorem~\ref{thm:obstruction} and
Corollary~\ref{cor:applies}.
...
\emph{Solved:} the two candidate algebras of [XIV, Rem.~9.1(a)] are ruled out, the first
because it is Morita-trivial and the second by Theorem~\ref{thm:obstruction}.
\emph{Open:} an algebra with $K_1$ of rank $\ge b_1$ and a natural map from $H^1(Y;\Z)$
(Remark~\ref{rem:needed}); and, unchanged from [XIV], the question whether $\cL$ has any
index-theoretic description at all, as opposed to the spectral-action and Albanese
descriptions it is already known to have [XIV, Cor.~7.2].
\emph{Impossible:} an odd pairing linear in $\alpha$ against a rank-one $K_1$, whenever
$b_1\ge2$. Together with [XIV, Prop.~2.1] this closes the naive routes in both the
...
(Proposition~\ref{prop:compacts}). The specification's own hint is the obstruction to
...
quadratic form (Theorem~\ref{thm:obstruction}); this is the same objection
[XIV, Prop.~2.1(iii)] raised against the finite-dimensional proposal, and it survives the
...
by Cramer's rule (Remark~\ref{rem:integrality}), and no index theorem is needed to see it.
\end{remark}
...
Theorem~\ref{thm:obstruction} bite. The winding number was computed before the theorem was
formulated, in order to find out what the available pairing actually measures.

\begin{thebibliography}{99}

\bibitem{II} R.~Chen, \emph{Algorithmic non-commutative class field theory, II: the trace
obstruction and the K-homological bulk}, 2026.

\bibitem{IV} R.~Chen, \emph{Algorithmic non-commutative class field theory, IV: the limit
module of abelian towers and the characteristic identity}, 2026.

\bibitem{VI} R.~Chen, \emph{Algorithmic non-commutative class field theory, VI: the shadow
algebra}, 2026.

\bibitem{IX} R.~Chen, \emph{Algorithmic non-commutative class field theory, IX:
exceptional zeros, the graph $\cL$-invariant, and the operator-algebraic
Mazur--Tate--Teitelbaum formula}, 2026.

\bibitem{XIV} R.~Chen, \emph{Algorithmic non-commutative class field theory, XIV: the
graph $\cL$-invariant as spectral action, Bloch effective mass and Albanese length, and
what the cyclic Chern character sees}, 2026.

\bibitem{Connes} A.~Connes, \emph{Noncommutative Geometry}, Academic Press, 1994.

\bibitem{PV} M.~Pimsner and D.~Voiculescu, \emph{$K$-groups of reduced crossed product
$C^*$-algebras by free groups}, J.\ Operator Theory \textbf{8} (1982), 131--156.
