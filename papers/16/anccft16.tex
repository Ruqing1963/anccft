% Algorithmic Non-commutative Class Field Theory, XVI
% The out-covering correspondence of the head map, its K_0 index eta_*, and the KK-category of the Iwahori tower
% Build: pdflatex anccft16 (x3)
...
\title[Algorithmic non-commutative class field theory, XVI]
{Algorithmic non-commutative class field theory, XVI:\\
the out-covering correspondence of the head map,\\ its $K_0$-index $\et$, and the
...
\subjclass[2020]{Primary 19K35, 46L80; Secondary 46L08, 11R23, 37B10}
\keywords{Kasparov $KK$-theory, $C^*$-correspondence, Hilbert module, graph algebra,
Cuntz--Krieger algebra, out-covering, Iwahori tower, degeneracy map}

\thanks{Paper XVI of the series; Papers I--XV are cited as [I]--[XV]. The verification
script \texttt{head\_correspondence.py} in the ancillary bundle reproduces every
computational claim; its output is archived as \texttt{head\_correspondence\_output.txt}.}

\begin{document}

\begin{abstract}
[VIII] realized the tail norm of the Iwahori tower on the shadow Kirchberg algebras
...
$\taup$ \emph{up} the tower. It left open [VIII, Rem.~2.5, 2.6, 5.1(a)] the mirror
...
homomorphism of [VIII] had failed. The pair $(\cX_\eta,\phi)$ is an injective, full,
...
$C^*$-bivariant form of the degeneracy calculus $\ta\etp=A_k$, $\etp\ta=A_{k+1}$ of [V],
...
Remark~\ref{rem:reversal}); the construction of $\cX_\eta$ and the computation of its index
are done in full. The in-covering homomorphism $\varphi_k$ and the matrix identities of
the tower are quoted from [VIII] and re-verified. Every finite identity was
machine-verified exactly on the $K_4$ tower, $q=2$, at the transitions
$k+1\to k$ for $k\le3$ (shadow graphs on $24,48,96$ vertices; Table~\ref{tab:battery}).
Section~\ref{sec:scope} records what is solved, what stays open, and where the
specification is corrected.

\section{Setting: the two degeneracy maps and the mirror problem}\label{sec:setting}

We keep the conventions of [VIII]. $Y_0$ is a finite connected $(q{+}1)$-regular graph,
...
pushforwards, $\taup=\ta^{t}$, $\etp=\et^{t}$ the pullbacks, with [V, Prop.~1.2]
...
\begin{equation}\label{eq:ktheory}
K_0(\cO_k)=\coker(I-B_k^{t})\cong\Z\oplus\Jac(Y_k),\qquad K_1(\cO_k)=\ker(I-B_k^{t})=\Z ,
\end{equation}
by the factorization $U(I-B_k^{t})V=\Delta_k^{t}\oplus(-I)$ of [VIII, Thm.~1.4] with
$U=\begin{psmallmatrix}I&-cI\\0&I\end{psmallmatrix}$,
$V=\begin{psmallmatrix}I&0\\-I&I\end{psmallmatrix}$.

[VIII] proved that $\tau$ extends to a graph morphism $\tau^{\sharp}\colon\Ysh{k+1}\to
...
$\varphi_k\colon\cO_k\to\cO_{k+1}$ [VIII, Lem.~2.2, Thm.~2.5], whose $K_0$-map is the
...
(Table~\ref{tab:battery}, line (O)), and the sum-over-lifts formula \emph{fails} to be a
...
partial isometry [VIII, Rem.~2.4]. Yet on $K_0$ it is the head pushforward $\et$, not
$\ta$, that descends:
\begin{equation}\label{eq:etdesc}
\et A_{k+1}^{t}=A_k^{t}\et,\qquad
H_k:=\begin{pmatrix}\et&0\\0&\et\end{pmatrix}\ \text{satisfies}\
H_k(I-B_{k+1}^{t})=(I-B_k^{t})H_k ,
\end{equation}
the first being the transpose of $\ta A_{k+1}=A_k\ta$ from \eqref{eq:calc};
whereas $\operatorname{diag}(\ta,\ta)$ does \emph{not} intertwine $I-B^{t}$
[VIII, Thm.~2.5(iv)]. So the arrow the tower asks for --- a morphism
...
\section{The out-covering correspondence}\label{sec:corr}
...
\begin{definition}[The head module]\label{def:module}
Let
\[
...
\begin{lemma}[The module is finitely generated projective and full]\label{lem:fgp}
...
\begin{theorem}[The left action]\label{thm:leftaction}
...
homomorphism of [VIII] are here mutually orthogonal (distinct out-arcs $p(f)\ne p(f')$ of
$p(w)$), so they add rather than interfere.
\end{proof}

\begin{definition}[The head correspondence and its Kasparov class]\label{def:class}
...
\section{The index is the head pushforward}\label{sec:index}

\begin{theorem}[$K_0$-index of the head correspondence]\label{thm:index}
The map $\otimes_{\cO_{k+1}}[\cE_\eta]\colon K_0(\cO_{k+1})\to K_0(\cO_k)$ is the head
pushforward: under \eqref{eq:ktheory} it is induced by $H_k=\operatorname{diag}(\et,\et)$,
i.e.\ the descent of $\et\colon\coker\Delta_{k+1}^{t}\to\coker\Delta_k^{t}$
\eqref{eq:etdesc}. On generators $[q_w]\mapsto[p_{\eta^{\sharp}(w)}]$; the $K_1$-map is
...
$\coker(I-B_{k+1}^{t})\to\coker(I-B_k^{t})$ is \eqref{eq:etdesc}; conjugating by $U$ of
[VIII, Thm.~1.4] (which commutes with $\operatorname{diag}$ blocks) turns it into $\et$ on
$\coker\Delta^{t}$. The $K_1$-map: $K_1(\cO_k)=\ker(I-B_k^{t})=\Z(\mathbf1,-\mathbf1)$ and
$H_k(\mathbf1,-\mathbf1)=q(\mathbf1,-\mathbf1)$ (Table~\ref{tab:battery}, line (K)). The last
statement is Lemma~\ref{lem:fgp}.
\end{proof}

\begin{proposition}[The correspondence is not invertible]\label{prop:notinv}
...
\begin{proof}
By reversal symmetry (Remark~\ref{rem:reversal}) $\et$ on $\Tor\coker\Delta_{k+1}^{t}\to
\Tor\coker\Delta_k^{t}$ is the mirror of the tail norm $\ta$ on $\Jac(Y_{k+1})\to\Jac(Y_k)$
of [VIII, Thm.~2.8]: surjective with kernel the $p$-torsion of order $p^{\,n_k(p-1)-1}$
(Table~\ref{tab:battery}, line (N)). A $\KK$-equivalence induces isomorphisms on $K_*$;
...
\begin{remark}[Reversal symmetry]\label{rem:reversal}
...
(Table~\ref{tab:battery}, line (R)): reversing all arrows exchanges $\tau\leftrightarrow\eta$,
$\Delta\leftrightarrow\Delta^{t}$, $\cO_{B}\leftrightarrow\cO_{B^{t}}$, and carries the
in-covering theory of [VIII] for $(\tau,\Delta)$ to the out-covering theory of this paper
...
\section{Kasparov products and the Hecke operators}\label{sec:products}
...
\begin{theorem}[Round trips are Hecke operators]\label{thm:hecke}
...
identities $\ta\etp=A_k$, $\etp\ta=A_{k+1}$ of {\rm[V, Prop.~1.2]}.
\end{theorem}
...
(Theorem~\ref{thm:index}, [VIII, Thm.~2.5]). Blockwise
$H_kT^{k}=\operatorname{diag}(\et\taup,\et\taup)$, and $\et\taup=(\ta\etp)^{t}=A_k^{t}$ by
transposing \eqref{eq:calc}; similarly $T^{k}H_k=\operatorname{diag}(\taup\et,\taup\et)$ with
$\taup\et=(\etp\ta)^{t}=A_{k+1}^{t}$ (Table~\ref{tab:battery}, line (H)). Both descend to
$\coker(I-B^{t})$, since each factor does.
\end{proof}

\begin{remark}[The opposite side and the full calculus]\label{rem:opposite}
...
composites $A_k$ and $A_{k+1}$ (Table~\ref{tab:battery}, line (B)). Thus all four
degeneracy maps of [V] are realized, two by $\KK$-classes on the $K_0$-side ($\taup$ up,
...
Pontryagin adjointness of [VIII, Lem.~2.1]. The correspondence $\cE_\eta$ supplies the one
arrow that was missing: the canonical downward morphism inducing $\et$.
\end{remark}

\begin{definition}[The $\KK$-category of the tower]\label{def:cat}
...
\begin{corollary}[Structure of $\mathcal{T}$]\label{cor:cat}
...
the operator-algebraic shadow of the two-sidedness of the pro-module $X_\infty$ of [V].
\end{corollary}

\begin{proof}
Theorems~\ref{thm:index}, \ref{thm:hecke}; injectivity/surjectivity from [VIII, Thm.~2.5]
and Proposition~\ref{prop:notinv}.
\end{proof}
...
\emph{Solved:} the open item [VIII, Rem.~5.1(a)] --- a canonical $C^*$-correspondence
$\cO_{k+1}\rightsquigarrow\cO_k$ induced by the out-covering $\eta^{\sharp}$ with $K_0$-map
$\et$. It is $\cE_\eta=(\cX_\eta,\phi)$ of Definition~\ref{def:class}: a finitely generated
projective, full, injective proper correspondence whose class in $\KK(\cO_{k+1},\cO_k)$
induces $\et$ (Theorem~\ref{thm:index}), completing the two-sided realization of the
degeneracy calculus (Theorem~\ref{thm:hecke}, Remark~\ref{rem:opposite}) and organizing the
levels into the $\KK$-category $\mathcal{T}$ (Definition~\ref{def:cat},
Corollary~\ref{cor:cat}). \emph{Open:} (a) the Cuntz--Pimsner algebra
$\cO_{\cX_\eta}$ of the head correspondence, and whether the inductive system it generates
gives a second model of the limit $\cO_\infty$ of [VIII] dual to the first; (b) an
explicit unbounded (spectral-triple) representative of $[\cE_\eta]$ pairing with the
$\cL$-invariant $KK$-classes of [XIV]; (c) the Hecke-module structure of $K_0(\cO_\infty)$
under the operators $[\varphi_k]\otimes[\cE_\eta^{(k)}]$ of Corollary~\ref{cor:cat}, and with
it [V, Prob.~5.1]. \emph{Impossible:} a unital $*$-homomorphism $\cO_{k+1}\to\cO_k$
inducing $\et$ (Section~\ref{sec:setting}: $\et[1]=q[1]$); and, as throughout the series, a
$\KK$-equivalence between adjacent levels (Proposition~\ref{prop:notinv}).
\end{remark}
...
$*$-homomorphism (Theorem~\ref{thm:leftaction}). This is the one place the geometry is
...
homomorphism of [VIII] could not. (4) The head pushforward $\et$, not $\ta$, is the map the
downward correspondence induces; $\ta$ descends on the opposite (Bowen--Franks) side, not on
$K_0$ (Remark~\ref{rem:opposite}). (5) ``All six degeneracy identities in $\KK$'' is made
precise as the Kasparov products of Theorem~\ref{thm:hecke} and the opposite-side identities
of Remark~\ref{rem:opposite}: the two Hecke operators are the two round-trip products, and
...
identities are quoted from [VIII] and re-verified exactly on $K_4$. The bivariant inputs
...
\bibitem{Bla} B.~Blackadar, \emph{$K$-theory for operator algebras}, 2nd ed., Math.\ Sci.\
Res.\ Inst.\ Publ.\ \textbf{5}, Cambridge Univ.\ Press, Cambridge, 1998.
\bibitem{Kas} G.~G.~Kasparov, \emph{The operator $K$-functor and extensions of
$C^*$-algebras}, Math.\ USSR-Izv.\ \textbf{16} (1981), no.~3, 513--572.
\bibitem{KP} J.~Kaminker and I.~Putnam, \emph{$K$-theoretic duality for shifts of finite
type}, Comm.\ Math.\ Phys.\ \textbf{187} (1997), 509--522.
