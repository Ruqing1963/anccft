% Algorithmic Non-commutative Class Field Theory, XV
% The parahoric factorization of the geodesic edge flow and the unitarity of the remainder
% Build: pdflatex anccft15 (x3)
...
\title[Algorithmic non-commutative class field theory, XV]
{Algorithmic non-commutative class field theory, XV:\\
the parahoric factorization of the geodesic edge flow,\\ the unitarity of the remainder,
...
\subjclass[2020]{Primary 11M41, 20E42; Secondary 05C50, 22E50, 37B10}
\keywords{$\widetilde A_2$ building, Ramanujan complex, zeta function, Hecke operator, parahoric
subgroup, geodesic edge flow, Kang--Li identity}

\thanks{Paper XV of the series; Papers I--XIV are cited as [I]--[XIV]. The verification
script \texttt{pgl3\_parahoric.py} in the ancillary bundle reproduces every computational
claim; its output is archived as \texttt{pgl3\_parahoric\_output.txt}.}

\begin{document}

\begin{abstract}
[XI, Cor.~3.5] found numerically, on one $\Atwo$ complex, that the characteristic polynomial of
...
[XI, Def.~3.1] is recorded (the geodesic condition is ``not in a common chamber''). Verified
...
complex of order $q$ ([X, Def.~1.1(i)--(iii)], vertex links the incidence graph of
$\PG(2,q)$), linear algebra over $\Q$, and the results of [XI], [XII] that are re-derived where
...
Corollary~\ref{cor:KL}); Lubotzky--Samuels--Vishne \cite{LSV} for the notion of Ramanujan
complex. The flag condition [X, Def.~1.1(iv)] is not assumed anywhere.

\section{Setting, and a correction}\label{sec:setting}
...
($K(e,e')=\#\{$chambers in which $e'$ follows $e\}$), all as in [XI, \S3] and [XII, Thm.~3.1].

\begin{definition}[Geodesic edge flow, chamber form]\label{def:LE}
$\LE\in M_{|E|}(\{0,1\})$, $\LE\bigl((x\to y),(y\to z)\bigr)=1$ iff $(y\to z)\in E$ and
$\{x,y,z\}\notin F$; equivalently $\LE=T^{t}S-K$ [XII, Thm.~3.1].
\end{definition}

\begin{remark}[Correction to {\rm[XI, Def.~3.1]}]\label{rem:corrXI}
[XI, Def.~3.1] used ``$z$ not adjacent to $x$''. The two conditions agree exactly when $Y$ is
a flag complex ([X, Def.~1.1(iv)]), which holds for $Y_{168}$, the only complex on which [XI]
evaluated $\LE$. On $Y_{21}$, whose $1$-skeleton is $K_{7,7,7}$, every $z$ is adjacent to $x$
and the adjacency version is the zero matrix, while Definition~\ref{def:LE} gives row sums
$q^{2}$; on $Y_{24}$ and $Y_{42}$ the two versions differ as well (Table~\ref{tab:battery},
line (D)). Definition~\ref{def:LE} is the projection of the geodesic operator of the
building, the operator $L_E$ of \cite{KL}, and every statement of [XI, \S3] and [XII] holds
for it with the proofs unchanged; the quoted numerics of [XI] concern $Y_{168}$ and are
unaffected.
\end{remark}
...
$N-(q{+}1)=q^{2}$. (b) is [XI, Lem.~3.2(b),(c)] with ``chamber'' in place of ``adjacent'', the
...
\section{Two-sided intertwining and the unitary remainder}\label{sec:intertwine}

Let $\Psi:=[S^{t}\ T^{t}\ M^{t}]\colon\ell^{2}(V)^{3}\to\ell^{2}(E)$ and
$W:=(\im\Psi)^{\perp}=\ker S\cap\ker T\cap\ker M$.

\begin{theorem}[Two-sided intertwining]\label{thm:two}
...
\begin{proof}
The first identity is [XI, Thm.~3.3], whose proof uses only Lemma~\ref{lem:local}(a),(b).
...
[XII, Thm.~3.1], and subtracting $\LE\Psi=\Psi C'$ gives $KS^{t}=(q{+}1)T^{t}$, $KT^{t}=M^{t}$,
$KM^{t}=T^{t}A_2+qS^{t}$, i.e.\ $\Ch$; $K^{t}=S^{t}T-\LE^{t}$ likewise gives $\Chp$. The two
product identities are Lemma~\ref{lem:local}(c): $(\LE\LE^{t})(e,e')$ counts common
...
four. Verified as exact integer identities on all five complexes (Table~\ref{tab:battery},
line (D)).
\end{proof}

\begin{theorem}[The remainder is $\sqrt q$ times an orthogonal matrix]\label{thm:unitary}
...
\begin{proof}
$W$ is invariant under $\LE$ and $\LE^{t}$ (Theorem~\ref{thm:two}), so the restriction of
...
eigenvalues of $R$ have modulus $\sqrt q$ to $10^{-12}$ (Table~\ref{tab:battery}, line (U)).
\end{proof}

\begin{remark}[What this says about the Riemann hypothesis]\label{rem:RH}
...
q^{-1/2}\}$. Theorem~\ref{thm:unitary} shows that the second clause of the edge criterion is
...
\section{The factorization theorem}\label{sec:factor}

\begin{theorem}[Parahoric factorization]\label{thm:factor}
...
\begin{proof}
By Theorem~\ref{thm:two}, $\LE=\LE|_{\im\Psi}\oplus R$, so $\det(I-u\LE)=\det(I-u\LE|_{\im\Psi})
...
$(I-uA_1+qu^{2}A_2)+qu\cdot(-q^{2}u^{2})=P(u)$ [XI, Prop.~3.4]. Multiply out. Degree and
roots: Theorem~\ref{thm:unitary}, $R$ invertible.
\end{proof}

\begin{proposition}[The kernel of $\Psi$]\label{prop:kernel}
The Gram matrix
\[
...
$\dim\ker\Psi\ge6$ [XI, Cor.~3.5(i)]. Say $Y$ is \emph{non-degenerate} (ND) if $\det H(\lambda)\ne0$
...
\]
and Theorem~\ref{thm:factor} is the identity of {\rm[XI, Cor.~3.5(ii)]}. $Y_{21},Y_{24},Y_{42},
Y_{168}$ satisfy (ND); the thin torus $T_6$ ($q=1$) does not (Table~\ref{tab:battery}, line (K)).
\end{proposition}

\begin{proof}
The block entries are [XII, Thm.~3.1]; $A_1$ is normal, so $\ell^{2}(V)$ is an orthogonal sum of
...
\section{The chamber companion and the Kang--Li identity}\label{sec:chamber}
...
\]
row sums $q$; it is the chamber flow $L_F$ of [XI, Rem.~3.6] and [XII, Def.~4.1]. Let
...
\begin{theorem}[Chamber companion]\label{thm:companion}
\[
...
\]
No commutation of $K,K^{t}$ is used. Exact on all five complexes (Table~\ref{tab:battery},
line (B)).
\end{proof}

\begin{theorem}[Kang--Li identity in companion form]\label{thm:KLcomp}
...
(Theorem~\ref{thm:two}). On $W$, $K=T^{t}S-\LE$ acts as $-R$ and $K^{t}$ as $-R^{t}$, so
$B|_{W^{3}}=\begin{psmallmatrix}0&0&-I\\qI&-R&-R^{t}\\0&-I&0\end{psmallmatrix}$ and, by the
elimination of Theorem~\ref{thm:companion},
$\det(I+uB|_{W^{3}})=\det\bigl((1+qu^{3})I-uR-u^{2}R^{t}\bigr)=\det\bigl((I-uR)(I-u^{2}R^{t})\bigr)
=G(u)G(u^{2})$, using $RR^{t}=qI$ (Theorem~\ref{thm:unitary}) and $\det(I-u^{2}R^{t})=G(u^{2})$.
...
\]
and Theorem~\ref{thm:factor} at $u$ and at $u^{2}$ turns the right side into the statement.
...
(Table~\ref{tab:battery}, line (B)).
\end{proof}

\begin{corollary}[Kang--Li's zeta identity]\label{cor:KL}
...
Theorem~\ref{thm:KLcomp}, $\det(I-u^{2}\LE)=\det(I-u^{2}\LE^{t})$, and the computations of
Table~\ref{tab:battery}; the equality $K_\Phi=G_\Phi$ and the general ratio are not proved
...
\quad adjacency version of [XI, Def.~3.1] equals the chamber version & $Y_{168},T_6$ yes; $Y_{21},Y_{24},Y_{42}$ no & exact\\
...
(B) $\LB\Phi=\Phi B$; Theorem~\ref{thm:KLcomp} at $3|E|+1$ points mod two primes; $\det(I+u\LB)=\det(I+uB)$ ($q=2$); ratio $(1-u^{3})^{\chi-n}$ on $T_6$ & all five ($Y_{168}$: random points) & exact / modular\\
...
\begin{sloppypar}
\emph{Solved:} [XI, Rem.~5.1(b)] in full --- the determinant identity for all $\Atwo$ complexes
(Theorem~\ref{thm:factor}, with the exact form of the trivial factor under (ND),
Proposition~\ref{prop:kernel}) and the identification of $G$ with Kang--Li's chamber factor
(Theorem~\ref{thm:KLcomp}: $G(u)G(u^{2})$ is the parahoric block of the chamber companion);
the Riemann hypothesis for the parahoric factor, unconditionally (Theorem~\ref{thm:unitary});
the Kang--Li identity itself in companion form, with a proof by the degeneracy calculus of
[XII] (Theorem~\ref{thm:KLcomp}). \emph{Open:} (a) a direct proof that $G_\Phi/K_\Phi=
(1-u^{3})^{\chi-n}$ (Corollary~\ref{cor:KL}), which would make the Kang--Li identity
...
\emph{Impossible or false as stated:} see Remark~\ref{rem:corrections}.
\end{sloppypar}
\end{remark}
...
every complex (Theorem~\ref{thm:unitary}); the Ramanujan property is exactly the vertex
condition (Remark~\ref{rem:RH}). (4) The formula requires (ND); on $T_6$ the trivial factor
has degree $15$ and $\deg G=15\ne4n+6$. (5) The ``block triangularization'' is in fact a
block diagonalization: $\im\Psi$ is invariant under $\LE^{t}$ too. (6) Our own [XI,
Def.~3.1] is corrected (Remark~\ref{rem:corrXI}). (7) The representation-theoretic reading of
...
and its companion for $q\ne2$ in Corollary~\ref{cor:KL}, quoted from \cite{KL}. The
modular certificates of Theorem~\ref{thm:KLcomp} are identities in $\F_p[u]$ for two
...
\bibitem{KLW} M.-H.~Kang, W.-C.~W.~Li and C.-J.~Wang, \emph{The zeta functions of complexes
from $\mathrm{PGL}(3)$: a representation-theoretic approach}, Israel J.\ Math.\ \textbf{177}
(2010), 335--348.
\bibitem{LSV} A.~Lubotzky, B.~Samuels and U.~Vishne, \emph{Ramanujan complexes of type
$\widetilde A_d$}, Israel J.\ Math.\ \textbf{149} (2005), 267--299.
\end{thebibliography}
