% Algorithmic Non-commutative Class Field Theory, X
% A~_2 complexes, higher simplicial Laplacians, and the fate of the trace obstruction in rank two
% Build: pdflatex anccft10 (x3)
...
\title[Algorithmic non-commutative class field theory, X]
{Algorithmic non-commutative class field theory, X:\\
$\Atwo$ complexes, higher simplicial Laplacians,\\ and the fate of the trace obstruction in
rank two}
...
\subjclass[2020]{Primary 05E45, 20E42; Secondary 46L80, 11F70, 05C50, 15B48}
\keywords{Bruhat--Tits building, $\Atwo$ complex, triangle presentation, Hodge Laplacian,
critical group, Hecke operator, companion matrix, trace obstruction, spectral conjecture,
Kirchberg algebra}

\thanks{Paper X of the series; Papers I--IX are cited as [I]--[IX]. The verification script
\texttt{pgl3\_building.py} in the ancillary bundle constructs every complex used and reproduces
every computational claim; its output is archived as \texttt{pgl3\_building\_output.txt}.}
...
arithmetic bridge of [I, Thm.~3.5] is carried instead by the \emph{Hecke--Laplacian}
...
exactly as in [II], but the trace obstruction of [II] does \emph{not} transfer: the $\Z/3$
...
critical group is open and unobstructed. Third, the shadow construction of [VI] extends to
arbitrary
...
Cuntz--Krieger facts quoted in [VI, VIII], the Cartwright--Mantero--Steger--Zappa theorem
...
not an input. Every finite identity was machine-verified exactly (Table~\ref{tab:battery});
...
the flag condition holds only for the largest of them, a distinction the theorems respect. Section~\ref{sec:scope}
records what is open and the corrections to the specification.

\section{\texorpdfstring{$\Atwo$ complexes of order $q$}{A2 complexes of order q}}\label{sec:complexes}

\begin{definition}[$\Atwo$ complex]\label{def:a2}
...
complexes. Thin complexes ($q=1$) are triangulated flat tori (Construction~\ref{con:thin}).
...
\begin{construction}[Thin tori]\label{con:thin}
...
\begin{construction}[Thick complexes of order $2$]\label{con:thick}
...
itself, exactly as the derived graphs of [II, \S5] in one dimension higher: vertices
...
conditions (i)--(iii) hold, with $|E|=|F|=7n$ (Table~\ref{tab:battery}, line (P)). The flag
...
$Y_{168}$ is an $\Atwo$ complex in the full sense of Definition~\ref{def:a2}, the others
...
every statement below depends only on the local structure of Definition~\ref{def:a2}, and
the statements that use (iv) are marked.
\end{construction}

\section{Hodge Laplacians and higher critical groups}\label{sec:hodge}
...
\begin{proposition}[Structure]\label{prop:hodge}
...
\section{The Hecke--Laplacian and the \texorpdfstring{$\mathrm{GL}_3$ $L$-values}{GL3 L-values}}\label{sec:hecke}

\begin{definition}[Cubic pencil and Hecke--Laplacian]\label{def:pencil}
...
\begin{theorem}[Hecke critical group and $L$-values]\label{thm:hecke}
\begin{enumerate}
...
[III, proof of Thm.~2.1, Step~2] applies verbatim to a matrix with two-sided kernel
...
\begin{theorem}[Companion reduction]\label{thm:companion}
...
This is [II, \S1.1] one degree higher.
\end{proof}

\section{The fate of the trace obstruction}\label{sec:trace}

\begin{theorem}[Grading]\label{thm:grading}
...
\begin{theorem}[No cubic trace obstruction]\label{thm:notrace}
For every $\Atwo$ complex of order $q$,
\[
\Tr\Cthree=0,\qquad\Tr(\Cthree)^{2}=0,\qquad
\Tr(\Cthree)^{3}=3|F|-3q|E|+3nq^{3}=n(q-1)^{2}(q+1)\ \ge\ 0,
\]
with equality only in the thin case $q=1$. Hence the mechanism of {\rm[II, Thms.~1.5--1.6]}
...
$\Tr A_1^{2}=0$ and $\Tr A_2=0$ by Theorem~\ref{thm:grading}, $\Tr A_1^{3}=3|F|$ and
$\Tr(A_1A_2)=|E|$ by Lemma~\ref{lem:counts}; substitute the counts:
...
the excess being the spurious closed $3$-walks (Table~\ref{tab:battery}).
\end{proof}

\begin{remark}[Why rank one is different]\label{rem:why}
In [II] the companion of $1-uT_p+qu^{2}$ has $\Tr C^{2}=\Tr T_p^{2}-2qn=n(q+1)-2qn<0$:
...
The obstruction of [II] is a rank-one accident of parity.
\end{remark}

\begin{definition}[Reduced pencil]\label{def:reduced}
By Theorem~\ref{thm:grading}, $\det P(u)\in\Z[u^{3}]$; write $\det P(u)=Q(u^{3})$ with
...
\begin{theorem}[Spectral realizability of the thick examples]\label{thm:realizable}
...
$m\ge m_0\in\{2,3\}$ (Lemma~\ref{lem:bound}); hence by \cite{KOR} there is a primitive
...
\begin{proof}
(i) is computation (Table~\ref{tab:battery}, line (R)); the grading statement is
Theorem~\ref{thm:grading}. (ii) The Boyle--Handelman conditions for a primitive integer
...
\begin{lemma}[Tail bound]\label{lem:bound}
...
\begin{remark}[What remains of the realization problem]\label{rem:realization}
Theorem~\ref{thm:realizable} is about the nonzero spectrum. The realization problem proper
...
rank one, Theorem~[II, 1.6], is unavailable in rank two, and that the necessary conditions
known to us are all satisfied. The problem is open and, as far as the trace method can see,
unobstructed. Whether the double-loop mechanism of Section~\ref{sec:shadow} can be upgraded
from a $K$-theoretic to a dynamical realization is the natural next question, and [II,
Thm.~1.6] no longer forbids it here.
\end{remark}

\section{Signed shadows and the higher Kirchberg algebras}\label{sec:shadow}

\begin{lemma}[Signed shadow lemma]\label{lem:signed}
...
witness of [VI, Thm.~3.1]; $K$-theory is the Cuntz--Krieger formula transported by $U,V$.
For $M=\Delta_p$ of a graph ($M_{ef}\le0$ off the diagonal) $\mathcal P=\emptyset$ and the
construction is [VI, Def.~1.1] with $c=M_{ee}-1$.
\end{proof}

\begin{theorem}[Higher shadow algebras]\label{thm:shadow}
For an $\Atwo$ complex $Y$ let $\cO(Y,\Delta_1):=\cO_{B(\Delta_1)}$ and
$\cO(Y,\DeltaH):=\cO_{B(\DeltaH)}$ be the algebras of Lemma~\ref{lem:signed} for $M=\Delta_1$
...
\]
the second under the hypothesis of Theorem~\ref{thm:hecke}(ii). Type- and
...
\begin{proof}
Lemma~\ref{lem:signed} with Proposition~\ref{prop:hodge} and Theorem~\ref{thm:hecke}; the
...
\begin{remark}[Digraph, not cellular algebra]\label{rem:digraph}
...
Lemma~\ref{lem:signed} realizes any integer matrix with positive diagonal as the
...
agree to $10^{-6}$ in every case (this also verifies \eqref{eq:lattice} itself, a
...
(H) Smith forms of $\Delta_0,\Delta_1,\Delta_2$; $K(Y)$; completeness via \eqref{eq:lattice} & thin, $Y_{21},Y_{24},Y_{42}$ & exact, complete\\
...
matrix shift equivalent over $\Z$ to $\Cthree$ (Remark~\ref{rem:realization}); (b) a
...
group arithmetic; (d) towers of $\Atwo$ complexes (the natural continuation of [III]--[IX]).
\emph{Impossible or false as stated:} a cubic trace obstruction (Theorem~\ref{thm:notrace}).
\end{remark}
...
(Theorem~\ref{thm:hecke}(iii)). (3) The test complexes are $\Atwo$ complexes of order $2$
...
(Remark~\ref{rem:digraph}). (5) The companion in the specification is correct, and its
reduction to $\DeltaH$ (Theorem~\ref{thm:companion}) is the rank-two form of [II, \S1.1].
\end{remark}
...
a triangle presentation), the Cuntz--Krieger facts quoted in [VI], and \cite{KOR} for
Theorem~\ref{thm:realizable} only. Spectral statements marked ``numeric'' in
Table~\ref{tab:battery} are floating-point computations with the stated tolerances; all
...
\bibitem{CMSZ} D.~I.~Cartwright, A.~M.~Mantero, T.~Steger and A.~Zappa, \emph{Groups acting
simply transitively on the vertices of a building of type $\widetilde A_2$, I}, Geom.\
Dedicata \textbf{47} (1993), 143--166.
\bibitem{DKM} A.~M.~Duval, C.~J.~Klivans and J.~L.~Martin, \emph{Critical groups of
simplicial complexes}, Ann.\ Comb.\ \textbf{17} (2013), 53--70.
