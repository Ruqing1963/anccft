% Algorithmic Non-commutative Class Field Theory, XI
% The PGL_3 positive Hecke-dynamical system: shift equivalence over Z and the geodesic edge flow
% Build: pdflatex anccft11 (x3)
...
\title[Algorithmic non-commutative class field theory, XI]
{Algorithmic non-commutative class field theory, XI:\\
the $\PGL_3$ positive Hecke-dynamical system:\\ shift equivalence over $\Z$ and the geodesic
edge flow}
...
\subjclass[2020]{Primary 37B10, 15B48; Secondary 46L80, 20E42, 11F70, 05E45}
\keywords{shift equivalence, primitive matrix, Generalized Spectral Conjecture, $\Atwo$
complex, Hecke operator, companion matrix, geodesic flow, Cuntz--Krieger algebra}

\thanks{Paper XI of the series; Papers I--X are cited as [I]--[X]. The verification script
\texttt{pgl3\_dynamical.py} in the ancillary bundle reproduces every computational claim; its
output is archived as \texttt{pgl3\_dynamical\_output.txt}.}
...
satisfies the Perron and net-trace conditions ([X], Kim--Ormes--Roush), its shift
...
of the bulk--boundary dichotomy of [I].
\end{abstract}
...
Table~\ref{tab:battery}). Theorem~\ref{thm:nocofactor} rests on exact rational Farkas
...
of [X]; the edge-flow theorem uses the flag condition [X, Def.~1.1(iv)], which holds for
$Y_{168}$ and fails for the smaller covers, so its numerical checks are on $Y_{168}$.
Section~\ref{sec:scope} records what is open and the corrections to the specification;
Proposition~\ref{prop:kms} answers its item on KMS states.

\section{The grading, the return map, and the cyclic reduction}\label{sec:grading}

Throughout, $Y$ is an $\Atwo$ complex of order $q$ in the sense of [X, Def.~1.1] with $n$
...
$P(u)=I-uA_1+qu^{2}A_2-q^{3}u^{3}I$ [X, Thm.~3.3].
...
among the three types [X, Lem.~1.2]. Verified as an exact block identity on four complexes
(Table~\ref{tab:battery}, line (G)).
\end{proof}

\begin{proposition}[The return map]\label{prop:P0}
The nonzero spectrum of $P_0$ is $\Lambda_Q$ of {\rm[X, Def.~4.4]}: one cube $\lambda^{3}$
...
\begin{lemma}[Cyclic lifting of shift equivalence]\label{lem:lift}
...
$B_0=P_0$ on four complexes (Table~\ref{tab:battery}, line (L)). The converse is the
restriction of the four equations to graded pieces.
\end{proof}

\section{Shift equivalence over \texorpdfstring{$\Z$}{Z}: criterion, cofactors, obstructions}\label{sec:se}

\begin{proposition}[Module criterion]\label{prop:module}
...
\begin{proposition}[Cofactor construction]\label{prop:cofactor}
...
$\Z$ to $P_0$, hence $\operatorname{cyc}(I,I,B)\SE\Cthree$ by Lemma~\ref{lem:lift}.
\end{proposition}

\begin{proof}
$\coker(tI-B_F)\cong\coker F$ by the companion reduction (as in [X, Thm.~3.3]), and
$\coker F=\coker\bigl(U(tI-P_0)\bigr)\cong\coker(tI-P_0)$ because $U$ is invertible over
$\Z[t^{\pm}]$; Proposition~\ref{prop:module}. For $U=tI-Y$: $\det(tI-Y)=t^{n}$ is a unit in
$\Z[t^{\pm}]$ when $Y$ is nilpotent, and $(tI-Y)(tI-P_0)=t^{2}I-(P_0+Y)t+YP_0$.
\end{proof}

\begin{theorem}[No nilpotent cofactor on $Y_{21}$, $Y_{24}$]\label{thm:nocofactor}
...
Proposition~\ref{prop:cofactor} with $K=1$ cannot produce a positive presentation.
\end{theorem}
...
\begin{remark}[The precise status of the realization problem]\label{rem:status}
Let $\Lambda$ be the nonzero spectrum of $P_0$. By [X, Thm.~4.5] and \cite{KOR}, $\Lambda$ is
...
(Lemma~\ref{lem:lift}) --- is exactly the \emph{weak form of the Generalized Spectral
...
(Lemma~\ref{lem:lift}), that the first explicit route is closed
(Theorem~\ref{thm:nocofactor}), and that a positive system carrying the Hecke spectrum ---
though not the Hecke module --- exists and is canonical (Section~\ref{sec:edge}).
\end{remark}

\section{The geodesic edge flow}\label{sec:edge}
...
\begin{definition}[Geodesic edge flow]\label{def:LE}
...
analogue of the non-backtracking condition of the Hashimoto matrix $\Anb$ of [I].
\end{definition}

\begin{lemma}[Local counts]\label{lem:local}
Let $Y$ satisfy {\rm[X, Def.~1.1(i)--(iv)]} and have vertex links isomorphic to the
...
\begin{theorem}[The edge flow carries the Hecke pencil]\label{thm:edge}
Under the hypotheses of Lemma~\ref{lem:local}, with
...
\end{align*}
by Lemma~\ref{lem:local}, i.e.\ $\LE g=(-q\gamma)\,f\circ t+\bigl(q^{2}\alpha f+\beta A_1f
...
$\LE\Psi=\Psi C'$ on $Y_{168}$ (Table~\ref{tab:battery}, line (I)).
\end{proof}

\begin{proposition}[$C'$ is the companion]\label{prop:Cprime}
...
\begin{corollary}[What the edge flow sees and what it does not]\label{cor:spectrum}
Under the hypotheses of Theorem~\ref{thm:edge}:
\begin{enumerate}
...
\begin{remark}[Kang--Li context]\label{rem:KL}
...
remainder of Corollary~\ref{cor:spectrum}(ii) is the parahoric spectrum, which for a
...
\begin{theorem}[The positive Hecke-dynamical algebra]\label{thm:algebra}
Let $Y$ satisfy the hypotheses of Theorem~\ref{thm:edge} and let $\cO_{\LE}$ be the
...
separable, nuclear and in the UCT class, by the criteria quoted in [VI, Thm.~3.1]; purely
...
module map is Theorem~\ref{thm:edge} and Proposition~\ref{prop:Cprime}. The last sentence is
Corollary~\ref{cor:spectrum}(ii) and [X, Thm.~3.3].
\end{proof}

\begin{proposition}[KMS states of the edge flow]\label{prop:kms}
...
of all nontrivial Satake data by Theorem~\ref{thm:edge}; the $\KMS$ \emph{state} sees only
$\lambda=q^{2}$. The Perron obstruction of {\rm[I, Thm.~2.5]} therefore holds verbatim in rank
...
$\LE$ (Theorem~\ref{thm:edge} gives them explicitly, $\Psi(f\circ t,f\circ h,m_f)$-combinations),
...
eigenvalue $e^{\beta}$ (as in [I, Thm.~2.4]); by Perron--Frobenius the only such eigenvalue is
$\rho(\LE)=q^{2}$ (row sums), with a unique normalized positive eigenvector. The rest is
Theorem~\ref{thm:edge} and the identity $\Tr(\LE^{k})=\sum_\lambda\lambda^{k}$.
\end{proof}

\begin{remark}[Bulk and boundary, in rank two]\label{rem:dichotomy}
[I, Thm.~3.7] found that the natural positive system of a graph --- the boundary algebra
$\cO_{\Anb}$ --- has $K$-theory blind to the Hecke data, while the Hecke critical group
$\Z\oplus\Jac(Y)$ lives in the Laplacian channel; [VI] realized the latter by a
Hecke-functorial, not Hecke-dynamical, Kirchberg algebra, and [II] showed no single algebra
can be both. In rank two the same dichotomy appears with one difference: the positive
system $\cO_{\LE}$ carries the whole nontrivial Hecke \emph{spectrum} (Theorem~\ref{thm:edge}),
and its $K_0$ is a finite group unrelated to $\JacH(Y)$, while $\cO(Y,\DeltaH)$ of [X] carries
$\Z\oplus\JacH(Y)$ without dynamics. An algebra carrying both would be $\cO_B$ for a
non-negative $B\SE\Cthree$; [II, Thm.~1.6] no longer forbids it (there is no trace
obstruction, [X, Thm.~4.2]), and its existence is the open instance of the weak Generalized
Spectral Conjecture of Remark~\ref{rem:status}. That is the exact residue of the problem.
\end{remark}
...
Generalized Spectral Conjecture over $\Z$ (Remark~\ref{rem:status}); (b) a proof of the
determinant identity of Corollary~\ref{cor:spectrum}(ii) for all $\Atwo$ complexes, and the
...
$K\ge2$ exist (Proposition~\ref{prop:cofactor}) --- the Farkas method extends but the
...
--- Theorem~\ref{thm:edge} carries it, as Hashimoto's matrix already did in rank one; what
...
whose three-step product cancels $-qA_2$ --- is exactly Lemma~\ref{lem:lift}: correct as a
...
Proposition~\ref{prop:cofactor}) is obstructed at degree two (Theorem~\ref{thm:nocofactor}).
...
the partition function (Proposition~\ref{prop:kms}); the ``curse'' of [II] is not broken but
...
parahoric spectrum (Table~\ref{tab:battery}, line (T)).
\end{sloppypar}
\end{remark}
...
Table~\ref{tab:battery}; Theorem~\ref{thm:nocofactor} is certified by exact rational
...
\bibitem{BH91} M.~Boyle and D.~Handelman, \emph{The spectra of nonnegative matrices via
symbolic dynamics}, Ann.\ of Math.\ (2) \textbf{133} (1991), 249--316.
\bibitem{BH93} M.~Boyle and D.~Handelman, \emph{Algebraic shift equivalence and primitive
matrices}, Trans.\ Amer.\ Math.\ Soc.\ \textbf{336} (1993), 121--149.
\bibitem{BS} M.~Boyle and S.~Schmieding, \emph{Strong shift equivalence and the generalized
spectral conjecture for nonnegative matrices}, Linear Algebra Appl.\ \textbf{498} (2016),
231--243; arXiv:1501.04697.
\bibitem{KL} M.-H.~Kang and W.-C.~W.~Li, \emph{Zeta functions of complexes arising from
$\mathrm{PGL}(3)$}, Adv.\ Math.\ \textbf{256} (2014), 46--103.
\bibitem{KOR} K.~H.~Kim, N.~S.~Ormes and F.~W.~Roush, \emph{The spectra of nonnegative
integer matrices via formal power series}, J.\ Amer.\ Math.\ Soc.\ \textbf{13} (2000),
773--806.
\end{thebibliography}
