% Algorithmic Non-commutative Class Field Theory, XIV
% The graph L-invariant as spectral action, Bloch effective mass and Albanese length; what the cyclic Chern character sees
% Build: pdflatex anccft14 (x3)
...
\title[Algorithmic non-commutative class field theory, XIV]
{Algorithmic non-commutative class field theory, XIV:\\
the graph $\cL$-invariant as spectral action,\\ Bloch effective mass and Albanese length,\\
and what the cyclic Chern character sees}

\author{Ruqing Chen}
\address{GUT Geoservice Inc., Rivi\`ere-Beaudette, Qu\'ebec, Canada}
\email{ruqing@hotmail.com}
\date{14 September 2026}
\subjclass[2020]{Primary 11R23, 58B34; Secondary 05C50, 19D55, 60J10, 81Q10}
\keywords{$\cL$-invariant, spectral triple, cyclic cohomology, spectral action, heat kernel,
Bloch band, crystal lattice, Albanese torus, random walk, exceptional zero}

\thanks{Paper XIV of the series; Papers I--XIII are cited as [I]--[XIII]. The verification
script \texttt{cyclic\_index.py} in the ancillary bundle reproduces every computational
claim; its output is archived as \texttt{cyclic\_index\_output.txt}.}

\begin{document}

\begin{abstract}
[IX] identified the graph $\cL$-invariant of an abelian voltage tower as
...
$(C(V)\oplus C(E^{+}),\ell^{2}(V)\oplus\ell^{2}(E^{+}),\Dp)$ of [II] the odd cyclic
...
\cite{CC,JLO,Co} supply names and context, not steps. Section~\ref{sec:scope} says what
...
The Hecke--Dirac operator of [II, Def.~3.1] is
\[
...
$\ker\partial=\ker\Dp\cap\ell^{2}(E^{+})$, and [IX, Def.~1.3]
\[
...
[IX, Table~1]; on $K_5$ ($q=3$, $\kappa_0=125$) they are $-\frac35$ for a single edge and
$-\frac75$ for a triangle (Table~\ref{tab:battery}).

\section{What cyclic cohomology sees, and what it cannot}\label{sec:nogo}

\begin{proposition}[Three impossibilities]\label{prop:nogo}
\begin{enumerate}
...
(Table~\ref{tab:battery}, line (C)). $\HH_1(\C^{N})=\Omega^{1}_{\C^{N}/\C}=0$ because
...
\begin{remark}[What the even Chern character does see]\label{rem:even}
...
the index of $\Dp$ localized at each idempotent --- the content of [II, Thm.~3.3]:
...
of the triple. Section~\ref{sec:action} says where.
\end{remark}

\section{The heat-kernel formulas}\label{sec:heat}

\begin{theorem}[Infrared, not ultraviolet]\label{thm:heat}
...
$\frac75+\frac85e^{-5t}$ (Table~\ref{tab:battery}, line (H), symbolic in $t$).
\end{proof}

\section{The \texorpdfstring{$\Z$}{Z}-cover, its Bloch bands, and the effective mass}\label{sec:bloch}

\begin{definition}[The $\Z$-cover of the tower]\label{def:cover}
...
graph $Y_k$ of the tower [IV, \S1] is $\widetilde Y_\alpha/\ell^{k}\Z$. Its Laplacian
...
are the \emph{Bloch bands}. The block-character decomposition [IV, Lem.~1.1] says
\[
...
\begin{theorem}[Effective mass]\label{thm:bloch}
...
\]
The exceptional zero of order two of $\Theta$ {\rm[IX, Thm.~1.2]} is the parabolic bottom of
...
\begin{proof}
$D(t^{-1})=D(t)^{t}$ [IX, Lem.~1.1] gives $\operatorname{spec}\Delta_{-\varphi}=
...
[IX, proof of Thm.~3.4] with $s=e^{i\varphi}-1$. The coefficient is computed exactly and
checked to $10^{-12}$ numerically on all six assignments ($\frac18,\frac14,\frac38,\frac3{25},\frac7{25}$;
Table~\ref{tab:battery}, line (B)).
\end{proof}

\section{The spectral action}\label{sec:action}
...
\begin{theorem}[$\cL$ as second variation of the spectral action]\label{thm:action}
...
first term analytic near $0$ by Theorem~\ref{thm:bloch}, the remainder smooth in $\varphi$
...
$t=10$ value agrees with $\cL$ to $10^{-14}$ (Table~\ref{tab:battery}, line (S)).
\end{proof}
...
side of the spectral triple; Proposition~\ref{prop:nogo} shows that side is blind to
...
\cite{KS1}; the graph statement is Theorem~\ref{thm:albanese} below. The Greenberg--Stevens
shape of [IX, Rem.~3.5] --- $\cL$ as the ratio of a second character derivative to a first
...
\begin{theorem}[$\cL$ as diffusion constant]\label{thm:diffusion}
...
on $K_5$, $\sigma^{2}=\frac3{50},\frac7{50}$ (Table~\ref{tab:battery}, line (V)).
\end{proof}
...
\section{Albanese length and integrality}\label{sec:albanese}
...
\begin{theorem}[$\cL$ as Albanese length]\label{thm:albanese}
\[
...
\]
which is the integrality $a_2\in\Z$ of {\rm[IX, Thm.~1.2(ii)]} seen from the cycle side.
\end{theorem}
...
$\det G=16,125$; $\kappa_0\cL=-8,-16,-24,-75,-175$ (Table~\ref{tab:battery}, lines (A),(T)).
\end{proof}

\begin{corollary}[Five faces of one number]\label{cor:faces}
...
(T) [IX, Thm.~1.2(ii)] on $K_5$: $a_2=-75,-175=-\kappa_0\|h_\alpha\|^{2}$ & $K_5$ & exact\\
\bottomrule
\end{tabular}}
\medskip
...
\begin{sloppypar}
\emph{Solved:} open item (a) of [IX, Rem.~5.1], ``a cyclic-cohomology formulation of
...
direction $\alpha$ (Theorem~\ref{thm:action}), the effective mass of the $\Z$-cover
(Theorem~\ref{thm:bloch}), its diffusion constant (Theorem~\ref{thm:diffusion}), and the
Albanese length of $[\alpha]$ (Theorem~\ref{thm:albanese}); the exceptional zero is the
...
crossed product $C_0(\widetilde Y_\alpha)\rtimes\Z$ or the Cuntz--Krieger algebra of [VIII],
pairs to $\|h_\alpha\|^{2}$ or to its integer multiple $\kappa_0\|h_\alpha\|^{2}$; the
integrality of Theorem~\ref{thm:albanese} makes the latter the only candidate for an index.
(b) The rank-two analogue for the $\widetilde A_2$ towers of [X]--[XII], where $b_1=0$ [XII]
and there is no $\alpha$: the Bloch picture survives (the Hecke pencil of [X] is a Floquet
determinant over a $2$-torus when a $\Z^{2}$-cover exists) but the $\cL$-invariant does not.
\emph{Impossible as stated:} Proposition~\ref{prop:nogo}, all three items.
\end{sloppypar}
\end{remark}
...
(Theorem~\ref{thm:heat}); the $t\to0^{+}$ side of the JLO cocycle is the topological Chern
character, which does not see $\alpha$. (5) The theorem that does hold is a second-variation
(curvature) statement for the spectral action, Theorem~\ref{thm:action}, and its
...
$\ell$ enters through [IX, \S2] alone. The battery adds $K_5$ ($q=3$) as asked.
\end{remark}
...
\bibitem{CC} A.~H.~Chamseddine and A.~Connes, \emph{The spectral action principle}, Comm.\
Math.\ Phys.\ \textbf{186} (1997), 731--750.
\bibitem{Co} A.~Connes, \emph{Noncommutative geometry}, Academic Press, San Diego, 1994.
\bibitem{JLO} A.~Jaffe, A.~Lesniewski and K.~Osterwalder, \emph{Quantum $K$-theory. I. The
Chern character}, Comm.\ Math.\ Phys.\ \textbf{118} (1988), 1--14.
\bibitem{KS1} M.~Kotani and T.~Sunada, \emph{Albanese maps and off diagonal long time
asymptotics for the heat kernel}, Comm.\ Math.\ Phys.\ \textbf{209} (2000), 633--670.
\bibitem{KS2} M.~Kotani and T.~Sunada, \emph{Standard realizations of crystal lattices via
harmonic maps}, Trans.\ Amer.\ Math.\ Soc.\ \textbf{353} (2001), 1--20.
\bibitem{Lo} J.-L.~Loday, \emph{Cyclic homology}, 2nd ed., Grundlehren Math.\ Wiss.\
\textbf{301}, Springer, Berlin, 1998.
\end{thebibliography}
