% Algorithmic Non-commutative Class Field Theory, VI
% The shadow algebra: a functorial Kirchberg realization of the critical group
% Build: pdflatex anccft6 (x3)
...
\title[Algorithmic non-commutative class field theory, VI]
{Algorithmic non-commutative class field theory, VI:\\
the shadow algebra --- a functorial Kirchberg realization\\ of the critical group}
...
\subjclass[2020]{Primary 46L80, 46L35; Secondary 05C50, 11G20, 37B10}
\keywords{Kirchberg algebra, Cuntz--Krieger algebra, critical group, sandpile group,
Hecke--Laplacian, Bowen--Franks group, functorial realization}

\thanks{Paper VI of the series; Papers I--V are cited as [I]--[V]. The verification
script \texttt{hecke\_kirchberg.py} in the ancillary bundle reproduces every
computational claim.}

\begin{document}

\begin{abstract}
The central open problem of this series --- posed in [III, Rem.~1.10] and sharpened
in [IV, \S6] --- asked for a Kirchberg $C^*$-algebra, functorial in the Hecke data of
...
[II] has no formulation on it; and it does not contradict [II], because $B(Y)$ shares
with the Ihara pencil only its Bowen--Franks data, not its spectrum --- the exact
reconciliation is the per-eigenvalue identity
$(1-2u)(1-u\lambda)-qu^2\big|_{u=1}=-(q+1-\lambda)$. What remains open, and what [II]
...
Theorems~\ref{thm:ktheory} and \ref{thm:kirchberg} are proved in full;
the unimodular factorization at the heart of Theorem~\ref{thm:ktheory} is a two-line
block computation reproduced below and machine-verified, together with the Smith-form
match against [I, Table~1], on $K_4$, $K_5$, $K_{3,3}$ and Petersen
(Table~\ref{tab:battery}). The classical inputs --- simplicity and pure infiniteness
criteria for Cuntz--Krieger/graph algebras and their $K$-theory --- are quoted with a
flagged citation. Section~\ref{sec:scope} states exactly what is solved, what is
open, and what is impossible; one error in the task specification that prompted this
paper is corrected there (Remark~\ref{rem:k1}).

\section{The shadow graph}\label{sec:shadow}
...
cokernel is $\Z\oplus\Jac(Y)$ with $\ker\Delta_p=\Z$ ([I, Thm.~3.5]).

\begin{definition}[Shadow graph]\label{def:shadow}
...
loophole left by the No-Go theorems of [II]. The obstruction
...
\section{The \texorpdfstring{$K$}{K}-theory}\label{sec:ktheory}

\begin{theorem}[Functorial realization]\label{thm:ktheory}
...
$\ker=\ker\Delta_p=\Z$ by [I, Thm.~3.5(i)]. The $K$-theory formula for graph
...
\section{The Kirchberg properties}\label{sec:kirchberg}

\begin{theorem}[Shadow algebra is Kirchberg]\label{thm:kirchberg}
...
obstruction of {\rm[II, Thms.~1.5--1.6]} therefore has no formulation on
$\cO_{B(Y)}$.
\end{theorem}
...
\section{Non-contradiction, and the exact scope of the solution}\label{sec:scope}

\begin{proposition}[Why this does not contradict the No-Go theorems]\label{prop:noncontra}
...
\begin{remark}[A correction to the task specification]\label{rem:k1}
The specification that prompted this paper asked for $K_1\cong\Z^{\,r}$. That is the
\emph{boundary}-channel invariant ([I, Thm.~3.7]), not the Laplacian-channel one:
the target fixed by [I, Thm.~3.5], [III, Thm.~1.x] and [IV] is
$K_1=\ker\Delta_p=\Z$, and that is what Theorem~\ref{thm:ktheory} delivers.
...
\begin{remark}[What is solved, what is open, what is impossible]\label{rem:scope}
\emph{Solved:} the single-level canonicity problem of [III, Rem.~1.10] --- a
...
functoriality --- whether the tail norms of [V, Thm.~3.1] lift to
...
what [II, Thm.~1.6] forbids for every non-negative presentation of every size. The
shadow algebra is Hecke-functorial, not Hecke-dynamical, and by [II] no algebra can
...
\toprule
$Y$ & $n$ & $q$ & (S) factorization \eqref{eq:factor} & $\Jac(Y)$ from
$\mathrm{SNF}(I-B^t)$ & vs [I, Table 1]\\
\midrule
...
factorization \eqref{eq:factor} holds as an exact matrix identity; the invariant
factors of $I-B(Y)^t$ reproduce [I, Table~1] exactly; strong connectivity, the
...
Unrefereed preprint. Theorems~\ref{thm:ktheory}--\ref{thm:kirchberg} and
Proposition~\ref{prop:noncontra} are proved; the two-line factorization
\eqref{eq:factor} can be checked by hand and was machine-verified on four graphs.
The classical Cuntz--Krieger inputs (graph-algebra $K$-theory; simplicity and pure
infiniteness criteria) are quoted through \cite{CK}. Remark~\ref{rem:scope} delimits the result: single-level canonicity
solved, tower functoriality open, Hecke-dynamical realization impossible by [II].

\begin{thebibliography}{9}
\bibitem{CK} J.~Cuntz and W.~Krieger, \emph{A class of $C^*$-algebras and
topological Markov chains}, Invent.\ Math.\ \textbf{56} (1980), 251--268; together
with the standard graph-algebra $K$-theory and simplicity/pure-infiniteness
criteria descending from it.
\end{thebibliography}
