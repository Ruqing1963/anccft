% Algorithmic Non-commutative Class Field Theory, II
% The Trace Obstruction and K-Homological Bulk
% Build: pdflatex anccft2 (x2)
...
\title[Algorithmic non-commutative class field theory, II]
{Algorithmic non-commutative class field theory, II:\\
the trace obstruction and the K-homological bulk}

\author{Ruqing Chen}
\address{GUT Geoservice Inc., Rivi\`ere-Beaudette, Qu\'ebec, Canada}
\email{ruqing@hotmail.com}
\date{8 September 2026}
\subjclass[2020]{Primary 46L80, 46L55; Secondary 11F70, 19K33, 37B10, 05C50}
\keywords{Cuntz--Pimsner algebra, trace obstruction, shift equivalence, Kasparov
$KK$-theory, Fredholm module, Ihara zeta function, critical group, Hecke correspondence}

\thanks{Paper II of the series begun in \emph{Algorithmic non-commutative class field
theory, I: the Langlands--thermodynamic bridge over Bruhat--Tits trees}. This paper
answers, negatively, the realization question left open there as Remark~3.11
(equivalently Remark~3.6 in some section numberings of the same manuscript): whether
the companion linearization of the Ihara pencil admits a positive $C^*$-correspondence
realizing it directly. Verification scripts are referenced inline; none require network
access.}
...
can be non-negative} (Theorem~\ref{thm:tracesame}) --- an unconditional, four-line fact.
...
eigenspaces (Lemma~\ref{lem:se}) --- hence preserves $\Tr(\,\cdot\,^2)$, closing the
realization problem of Paper I, Remark~3.11, in the negative for matrices of every size
(Theorem~\ref{thm:traceany}).
...
index (Section~\ref{sec:barrier}); if one existed, it would trivialize the realization
...
$\ind(\mathsf D_p)=\Z\oplus\Jac(Y)$ exactly (Theorem~\ref{thm:index}), inside
...
Theorem~\ref{thm:tracesame} is proved here in full, elementarily, using nothing beyond
the fact that trace is a similarity invariant and that non-negative matrices have
non-negative squared-trace. Theorem~\ref{thm:traceany}, which rules out the auxiliary-vertex and Drinen--Tomforde
...
elementary Lemma~\ref{lem:se} by a two-way injection of generalized eigenspaces, and no
external citation is load-bearing for it. Theorem~\ref{thm:index} is proved in
full and requires no citation beyond standard finite-dimensional linear algebra dressed
in Fredholm-module language. Section~\ref{sec:tower} is exploratory: it states a
research programme, not a theorem, and we say so throughout.

\section{The trace obstruction}\label{sec:trace}
...
\begin{lemma}\label{lem:tracecomp}
...
This has been verified directly on the eleven graphs of Table~\ref{tab:trace}, in every
...
\begin{remark}
Theorem~\ref{thm:tracesame} needs nothing beyond Lemma~\ref{lem:tracecomp} and the
...
\begin{lemma}[Shift equivalence preserves the nonzero spectrum]\label{lem:se}
...
\begin{theorem}[Unconditional obstruction: no dilation of any size]\label{thm:traceany}
...
\begin{proof}
By Lemma~\ref{lem:se}, shift equivalence would force
...
An earlier draft stated Theorem~\ref{thm:traceany} conditionally on the resolution of
...
shift equivalence, is the elementary Lemma~\ref{lem:se}. The theorem is unconditional
...
\begin{remark}[What is, and is not, ruled out]\label{rem:scope}
Theorem~\ref{thm:traceany} rules out any \emph{faithful} dilation of $C$ --- one
...
\section{The virtual correspondence barrier}\label{sec:barrier}

Given Theorems~\ref{thm:tracesame}--\ref{thm:traceany}, it is natural to ask whether the
...
\begin{proposition}\label{prop:nograding}
...
matrix problem addressed by Theorems~\ref{thm:tracesame}--\ref{thm:traceany}, trivializing
...
Theorem~\ref{thm:tracesame} shows this difference is not, for any $q\ge2$, again the
...
\section{The K-homological bulk, and the congruence tower}\label{sec:tower}

\subsection{The Hecke--Dirac operator and its index}

\begin{definition}[Hecke--Dirac operator]\label{def:dirac}
...
\begin{theorem}[Fredholm module and index]\label{thm:index}
...
Lemma~\ref{lem:laplacian} and Theorem~3.5(i) of Paper I.
\end{proof}

\begin{remark}[The honest scope of this theorem]\label{rem:scope2}
...
that $KK$-theory demands, and Theorem~\ref{thm:tracesame} shows this is not a matter of
...
\begin{remark}[Finite-dimensional triviality]\label{rem:trivial}
...
\subsection{The congruence tower: where new content could appear}\label{subsec:tower}

The finite-dimensionality identified in Remark~\ref{rem:trivial} is specific to working
...
\begin{conjecture}[Congruence tower and the profinite $KK$-limit]\label{conj:tower}
...
dictated by our own data (Section~\ref{sec:abtower}), not by preference:
\begin{itemize}
\item[(ii-a)] \emph{Abelian deck towers.} For towers carrying a genuine
$\Z_p$-deck action (the voltage towers of Table~\ref{tab:tower}), the limit
...
$\lambda_{\mathrm{meas}}=\lambda(g)-1$ of Remarks~\ref{rem:towerhonest} and
\ref{rem:newton} hold to the integer in all four computed towers --- but the
...
Iwahori path tower of Remark~\ref{rem:iwahori}, exhibits the exact pseudo-Iwasawa law
...
\end{itemize}
\item[(iii)] the trace obstruction of Theorem~\ref{thm:traceany} persists at every finite
...
\begin{remark}
We state Conjecture~\ref{conj:tower} as a research programme, not a theorem, and we have
...
Theorem~\ref{thm:traceany} should persist as non-negativity of the $\tau$-weighted
...
Section~\ref{sec:trace}, proved level-by-level, survive or dissolve in the projective
...
closed form of Lemma~\ref{lem:tracecomp}, for every graph of Paper I's Table~1. Exact
...
\texorpdfstring{$\Z_\ell$}{Z-l}-covers}\label{sec:abtower}

Conjecture~\ref{conj:tower} concerns the congruence tower $\Gamma_0(p^k)\backslash X$,
...
particular the obstruction density of Theorem~\ref{thm:traceany} is constant along the
...
\begin{remark}[What the table shows, and what it does not]\label{rem:towerhonest}
...
whatever the correct formulation of Conjecture~\ref{conj:tower}(ii) for the congruence
...
$\Z_p[[T]]$-form of part (ii) of Conjecture~\ref{conj:tower} as stated above}: whatever
...
as the abelian-deck statement, which the data of Table~\ref{tab:tower} and the Newton
...
\label{rem:newton}
In the four abelian towers of Table~\ref{tab:tower} the characteristic-element mechanism
...
\begin{remark}[Relation to Conjecture~\ref{conj:tower}]
...
This is an unrefereed preprint. Theorem~\ref{thm:tracesame} and Theorem~\ref{thm:index}
are proved here in full from elementary linear algebra and require no external citation.
Theorem~\ref{thm:traceany} depends on the claim that shift equivalence forces agreement
...
further citation. Proposition~\ref{prop:nograding} is an argument, not a theorem in the
...
Theorems~\ref{thm:tracesame}--\ref{thm:traceany}) rather than a proof ruling out every
conceivable construction. Conjecture~\ref{conj:tower} is entirely open; none of its three
parts is established here, and part (iii) is explicitly posed as the paper's main
unresolved question. All computations of Section~\ref{sec:computations} are exact and
were checked independently of the general proof of Lemma~\ref{lem:tracecomp}. The tower
computations of Section~\ref{sec:abtower} are exact integer computations, cross-checked
...
structural refinement of Remark~\ref{rem:towerhonest}(2) is known to their authors
...
\bibitem{Williams} R.~F.~Williams, \emph{Classification of subshifts of finite type},
Ann.\ of Math.\ (2) \textbf{98} (1973), no.~1, 120--153; errata ibid.\ \textbf{99}
(1974), no.~2, 380--381. [Main paper Crossref-verified (DOI 10.2307/1970908; Crossref
records the start page only, the end page 153 is the standard record); errata
primary-source verified: JSTOR, DOI 10.2307/1970903.]
\bibitem{Pimsner} M.~V.~Pimsner, \emph{A class of $C^*$-algebras generalizing both
Cuntz--Krieger algebras and crossed products by $\Z$}, in: Free probability theory
(Waterloo, ON, 1995), Fields Inst.\ Commun.\ \textbf{12}, Amer.\ Math.\ Soc.,
Providence, RI, 1997, pp.~189--212. [Crossref-verified: DOI 10.1090/fic/012/08,
pp.~189--212; Crossref dates the chapter December 1996, the volume is conventionally
cited 1997.]
\bibitem{Sunada} T.~Sunada, \emph{Unitary representations of fundamental groups and
the spectrum of twisted Laplacians}, Topology \textbf{28} (1989), no.~2, 125--132.
[Crossref-verified: DOI 10.1016/0040-9383(89)90015-3.]
\bibitem{Levine} L.~Levine, \emph{Sandpile groups and spanning trees of directed line
graphs}, J.\ Combin.\ Theory Ser.\ A \textbf{118} (2011), no.~2, 350--364.
...
DOI 10.1007/s40316-020-00152-4. Both earlier out-of-band page ranges (335--359 and
133--152) were incorrect.]
\bibitem{McGownVallieres} K.~J.~McGown and D.~Vallières, \emph{On abelian
$\ell$-towers of multigraphs II}, Ann.\ Math.\ Qu\'ebec \textbf{47}, no.~2, 461--473.
[Crossref-verified: DOI 10.1007/s40316-021-00183-5; Crossref carries an online date of
November 2021 and an issue date of October 2023 for this record. Both earlier
out-of-band venues (J.\ Number Theory \textbf{238} and Ann.\ Math.\ Qu\'ebec
\textbf{46}) were incorrect. A third part in the series exists and is not verified
here.]
...
DOI 10.5802/alco.225. This supersedes the earlier entry entirely: the previously
listed title (``Jacobians of finite and infinite voltage covers of graphs'') and venue
(J.\ Algebraic Combin.) were incorrect in \emph{both} out-of-band passes --- note that
Algebraic Combinatorics and J.\ Algebraic Combin.\ are distinct journals.]
\end{thebibliography}
