% Algorithmic non-commutative class field theory, XVIII
% Build: pdflatex anccft18 (x3)
...
\title[Algorithmic non-commutative class field theory, XVIII]
{Algorithmic non-commutative class field theory, XVIII:\\
the unitary remainder in rank $d$}

\author{Ruqing Chen}
\address{GUT Geoservice Inc., Rivi\`ere-Beaudette, Qu\'ebec, Canada}
\email{ruqing@hotmail.com}
\date{19 September 2026}
\subjclass[2020]{Primary 51E24, 05E45; Secondary 20E42, 11F72, 05C50}
\keywords{affine building, $\widetilde A_d$ complex, geodesic edge flow, projective space,
Ihara zeta function, Ramanujan complex}
...
\section{The link, and what the types force}\label{sec:link}

Let $X$ be an $\Atilde d$ complex of order $q$ in the sense of [X, Def.~1.1] generalised to
...
\]
the rank-$d$ form of [XV, Def.~1.1]; for $d=2$ a $2$-simplex is a chamber and this is
[XV, Def.~1.1] verbatim, including the correction recorded there.

\begin{lemma}[What the typing forces]\label{lem:typing}
...
\begin{theorem}[Unitarity identity in rank $d$]\label{thm:main}
...
Lemma~\ref{lem:typing}: the block is $BB^{\mathsf T}$ with $B[x][z]=1$ iff the point $z$
...
\begin{remark}[Reduction to Paper XV]\label{rem:d2}
At $d=2$ the constants are $q^{\,d-1}=q$ and $q^{\,d-1}(q-1)=q^2-q$, so
Theorem~\ref{thm:main} is [XV, Thm.~2.1] and its proof is [XV, Lem.~1.3] with the
counting done once instead of case by case. Check~(A) verifies the two identities, with
these constants, on the complexes $Y_{21}$, $Y_{24}$ and $Y_{42}$ of [X, Constr.~1.4].
\end{remark}

\begin{proposition}[The mixed identities]\label{prop:mixed}
...
\begin{corollary}[Unconditional consequences]\label{cor:uncond}
...
$T\LE w=q^{\,d}Sw=0$ by Proposition~\ref{prop:mixed}, and
$\|\LE w\|^2=\langle\LE^{\mathsf T}\LE w,w\rangle=q^{\,d-1}\|w\|^2$ by
Theorem~\ref{thm:main}; both spaces have dimension $|E|-n$, so the isometry is onto.
\end{proof}

\begin{corollary}[The critical circle in rank $d$]\label{cor:circle}
...
\begin{proof}
Immediate from Theorem~\ref{thm:main}, since $T^{\mathsf T}T$ and $S^{\mathsf T}S$ vanish
on $W$; an orthogonal matrix has all eigenvalues of modulus one.
\end{proof}

\begin{remark}[The typing is not a convention]\label{rem:typing}
The uniformity in Theorem~\ref{thm:main} is a property of the hyperplane/point pairing, not
...
moduli rather than one. Lemma~\ref{lem:typing} is what rules this out.
\end{remark}

\section{Verification}\label{sec:battery}

The script \texttt{ad\_unitarity.py} runs the four checks of Table~\ref{tab:battery} in
...
(A) & the identities of \S\ref{sec:identity} on $Y_{21},Y_{24},Y_{42}$, with $d=2$ & exact\\
...
$781$. Check~(A) builds the thick $\Atilde2$ complexes of [X] and verifies
Theorem~\ref{thm:main}, Proposition~\ref{prop:mixed} and
Corollary~\ref{cor:uncond}(ii) on each; check~(S) finds $126$, $144$ and $252$ singular
values equal to $\sqrt2$ out of $147$, $168$ and $294$, which are exactly $|E|-n$.

\section{Status, scope, corrections}\label{sec:scope}

\emph{Proved.} Lemma~\ref{lem:typing}, Theorem~\ref{thm:main},
Proposition~\ref{prop:mixed} and Corollary~\ref{cor:uncond} for every $\Atilde d$ complex
of order $q$, $d\ge2$; Corollary~\ref{cor:circle} for every such complex admitting an
invariant $W\subseteq\ker S\cap\ker T$.

\emph{Not proved, and this is the gap.} The existence of that $W$. For $d=2$ it is
[XV, Thm.~2.1], obtained from the intertwining $\LE\Psi=\Psi C'$ with
$\Psi=[S^{\mathsf T}\ T^{\mathsf T}\ M^{\mathsf T}]$ and the cubic Hecke pencil.
Corollary~\ref{cor:uncond} is what survives unconditionally: $\LE$ is already
...
\begin{remark}[The candidate intertwiner in rank $d$]\label{rem:psi}
...
[XV, Thm.~2.1] and the block count $d+1$ matches the degree of the Hecke pencil. We record
...
satisfy the hypothesis of Corollary~\ref{cor:circle} and complete the argument. Both
...
Corollary~\ref{cor:circle} is unconditional for every $d\ge2$, and at $d=2$ the
...
\emph{Solved:} the counting half of [XV, Rem.~6.1(d)] --- the exact decomposition of
...
Corollary~\ref{cor:circle} is unconditional; what remains open there is the exact rank of
$\Psi_d$, hence $\dim W_d$;
...
property, the other half of [XV, Rem.~6.1(d)], which this note does not touch.
...
complex puts hyperplanes against points (Lemma~\ref{lem:typing}), and the only geometric
input is that two distinct hyperplanes meet in codimension two. With intermediate
dimensions the count is not constant (Remark~\ref{rem:typing}), so an identity of the
stated shape would be false. (2) The specification asked for
$W_d=\bigcap_{i=0}^{d}\ker S_i$. Only two operators appear in Theorem~\ref{thm:main}, and
...
correct, and are Corollary~\ref{cor:circle}. The third task --- the explicit $\Atilde3$
...
presentations over $\PG(2,2)$ [X, Constr.~1.4]; there is no comparably small search in
...
Unrefereed preprint. Theorem~\ref{thm:main} and everything derived from it are proved in
...
Remark~\ref{rem:typing} was computed before the positive half was proved, to check that
the question had teeth.

\begin{thebibliography}{99}

\bibitem{X} R.~Chen, \emph{Algorithmic non-commutative class field theory, X:
$\widetilde A_2$ complexes, higher simplicial Laplacians, and the fate of the trace
obstruction in rank two}, 2026.

\bibitem{XI} R.~Chen, \emph{Algorithmic non-commutative class field theory, XI: the
$\mathrm{PGL}_3$ positive Hecke-dynamical system}, 2026.

\bibitem{XV} R.~Chen, \emph{Algorithmic non-commutative class field theory, XV: the
parahoric factorization of the geodesic edge flow, the unitarity of the remainder, and the
chamber companion}, 2026.
...
$\widetilde A_2$}, Geom.\ Dedicata \textbf{47} (1993), 143--166.
