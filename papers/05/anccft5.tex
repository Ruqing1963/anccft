% Algorithmic Non-commutative Class Field Theory, V
% The degeneracy structure of the Iwahori tower and the Euler correction
% Build: pdflatex anccft5 (x3)
...
\title[Algorithmic non-commutative class field theory, V]
{Algorithmic non-commutative class field theory, V:\\
the degeneracy structure of the Iwahori tower\\ and the Euler correction}
...
\subjclass[2020]{Primary 05C50, 11R23; Secondary 20C08, 05C25, 46L80}
\keywords{Iwahori--Hecke algebra, degeneracy maps, line digraph, sandpile group,
non-commutative Iwasawa theory, Euler characteristic}

\thanks{Paper V of the series; Papers I--IV are cited as [I]--[IV]. All computational
claims are reproduced by the staged scripts \texttt{iw\_stage1--3.py} in the ancillary
bundle. For the fixed tower over $K_4$ every ``verified'' statement below is a finite
exact-integer computation and is therefore a proof for that tower; general-$Y_0$
statements are proved generally where so marked.}

\begin{document}

\begin{abstract}
The congruence-model (Iwahori path) tower $Y_0\leftarrow Y_1\leftarrow\cdots$ of
[II, \S5] is non-normal, carries no deck action, and obeys the exact pseudo-Iwasawa
law $\nu_p=\mu p^k-k+\nu_0$ with $\lambda=-1$, impossible over any commutative
$\Zp[[T]]$ base ([II, Rem.~5.2], [III, \S2]). This paper equips that tower with its
...
bottom, $\ta\etp=T_p$ of [I]) and $\ta\taup=\et\etp=qI$. Theorem~\ref{thm:euler}
...
the $-\chi$ of [III, Thm.~2.4] is the $H^0$ of this complex. Theorem~\ref{thm:norm}
...
$q^{\,n_k(q-1)-1}$ forced by [III, Thm.~2.1], and hence assembles the tower into one
...
genuinely two-sided. Proposition~\ref{prop:nonnormal} records a second, independent
...
base must reproduce, and we decline --- for the same reason as in [III] --- to define
...
Theorems~\ref{thm:euler} and \ref{thm:norm} and Propositions~\ref{prop:calculus},
\ref{prop:mirror} are proved for a general Eulerian $q$-regular strongly connected
level $\ge1$ (the base level $k=0$ is the boundary case, treated in
Remark~\ref{rem:level0}); every identity was additionally machine-verified exactly on
the $K_4$ tower at all levels $k\le3$ (Table~\ref{tab:battery}).
Proposition~\ref{prop:nonnormal} is a finite exact computation on the fixed tower,
hence a proof for it; no general-graph claim is made. Section~\ref{sec:axioms} proves
nothing and says so: it is a specification, and Problem~\ref{prob:char} is open.

\section{The degeneracy pair and its incidence calculus}\label{sec:calculus}
...
$n_k=n(q+1)q^{k-1}$ ([II, \S5], [III, \S2]).

\begin{definition}[Degeneracy maps]\label{def:degen}
...
\begin{proposition}[Incidence calculus]\label{prop:calculus}
For every $k\ge1$:
\[
...
\begin{proof}
Entrywise bookkeeping of tails and heads; the middle two are [III, Thm.~2.1, Step~1],
the outer two their transposes, the last two the regularity of degrees. The boundary
identity is $S\,T^{\,t}=T_p$ in the notation of [I, Thm.~3.7]. All six verified as
exact matrix identities at $k=1,2$ (Table~\ref{tab:battery}).
\end{proof}

The pair $(\ta,\et)$ is the graph avatar of the modular degeneracy pair
$X_0(p^{k+1})\rightrightarrows X_0(p^k)$, and Proposition~\ref{prop:calculus} is the
...
\section{The degeneracy complex and the Euler rank}\label{sec:euler}

\begin{theorem}[Euler rank of the degeneracy complex]\label{thm:euler}
...
and the correction $-\chi\cdot k=-k$ of {\rm[III, Thm.~2.4]} is the accumulated
...
invariants at $k=1,2$ (Table~\ref{tab:battery}).
\end{proof}

\section{The tail norm and the pro-module}\label{sec:norm}

\begin{theorem}[Tail norm]\label{thm:norm}
...
$q^{\,n_k(q-1)-1}$, the increment proved in {\rm[III, Thm.~2.1]};
...
$\ord_p=\mu p^k-k+\nu_0$, $\mu=n(q+1)/q$, of {\rm[III, Thm.~2.2]} --- now the size
...
Proposition~\ref{prop:calculus}; subtract from $q\ta$. (ii) Every level-$k$ vertex
extends, so $\ta$ hits every basis vector; $\deg\circ\ta=\deg$ since fibres are
summed; torsion-surjectivity then follows exactly as in [IV, Lem.~1.3] (the kernel of
$\deg$ is the torsion), and the kernel order is forced by counting against
[III, Thm.~2.1]. (iii) Inverse limits over surjections of compact modules; the Hecke
...
the orders $7,18$ matching [II, Rem.~5.2] (Table~\ref{tab:battery}).
\end{proof}

\begin{proposition}[Head mirror, and the exact asymmetry]\label{prop:mirror}
...
degeneracy boundary of Theorem~\ref{thm:euler}. The tower is thus canonically
...
\begin{remark}[The boundary level]\label{rem:level0}
...
[I, Rem.~1.8], the shared Perron root of [II], the excluded trivial eigenvalue of
[III] --- reappears here as the unique level where the norm formalism acquires a
correction term, and the correction is once more $\partial$.
\end{remark}

\begin{proposition}[Second non-commutativity certificate]\label{prop:nonnormal}
...
certificate $\lambda=-1<0$ of {\rm[III, Prop.~2.3]}, this gives two independent
...
\section{What a characteristic theory must specialize to}\label{sec:axioms}
...
proliferation of {\rm[III, Thm.~2.2]};
\item[(A2)] an additive Euler term $-\chi\cdot k$ with
$\chi=\rk\ker(\taup-\etp)$, the $H^0$ of the degeneracy complex
(Theorem~\ref{thm:euler}) --- intrinsic to the base, external to any commutative
characteristic ideal;
\item[(A3)] on sub-towers carrying an abelian deck action, reduction to the
commutative characteristic ideal $\bigl(\det D(1+T)\bigr)$ of {\rm[IV, Thm.~2.1]},
with the Newton-polygon evaluation of {\rm[II, Rem.~5.4]}.
\end{enumerate}

\begin{problem}\label{prob:char}
Construct $\mathrm{Char}$ satisfying (A1)--(A3) on the pro-module
$(X_\infty,A_\infty,\tau,\eta)$ of Theorem~\ref{thm:norm}.
\end{problem}
...
Proposition~\ref{prop:nonnormal}, generated by degeneracy correspondences rather
...
([III, Rem.~2.6]). What Papers~II--V have done instead is to make
Problem~\ref{prob:char} well-posed: the category, the operators, the two invariants
...
Proposition~\ref{prop:nonnormal} proved for the fixed $K_4$ tower by exact
computation and asserted for no other graph. Section~\ref{sec:axioms} is a
specification; Problem~\ref{prob:char} is open, and the paper's deliberate refusal to
...
[Primary-source verified: the IH\'ES journal page, Volume 101 (2005), pp.~163--208,
authors and title as listed.]
\end{thebibliography}
