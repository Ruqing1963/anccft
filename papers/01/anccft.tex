% Algorithmic Non-commutative Class Field Theory I
% The Langlands--Thermodynamic Bridge over Bruhat--Tits trees
% Sections 1-3.  Build:  pdflatex anccft (x2)
...
% ---- macros (compatible with the macro union of the localized Bost--Connes series)
...
\title[Algorithmic non-commutative class field theory, I]
{Algorithmic non-commutative class field theory, I:\\
the Langlands--thermodynamic bridge over Bruhat--Tits trees}
...
\subjclass[2020]{Primary 46L55, 11F70; Secondary 46L80, 11F72, 20E08, 37A55, 11G20}
\keywords{Bost--Connes system, Bruhat--Tits tree, Hecke groupoid, KMS states,
Satake parameters, Ihara zeta function, Cuntz--Pimsner algebra, Smith normal form,
critical group, Langlands correspondence}

\thanks{Paper I of a new series, logically independent of the thirty-nine papers of
\emph{Localized Bost--Connes systems}; that monograph is used only as a source of
comparison statements, and every such statement is quoted in place. Verification scripts
for Section~\ref{sec:ktheory} are in \texttt{code/hecke-snf/}.}
...
Section~\ref{sec:geometry} constructs the state space $\Omega$ (the space of lattice
...
Section~\ref{sec:thermo} inserts the Satake parameters into the Radon--Nikodym cocycle.
...
Section~\ref{sec:ktheory} is the algorithmic bridge, and it is where the higher-weight
...
(Theorem~\ref{thm:gap}). Paper XXI is the case $w=2$; the abelian theory of the monograph
...
and it is the reason Section~\ref{sec:ktheory} exists: $K$-theory carries no positivity
...
marked $[\mathrm{V}]$ and the verification table is Table~\ref{tab:snf}; in the present
version no numbered statement carries this label, Theorem~\ref{thm:blind} having been
promoted to a proof, and Table~\ref{tab:snf} now serves as independent confirmation
...
particular Theorem~\ref{thm:kms} is a theorem, its extension to ramified and to $\GL_n$
data is Conjecture~\ref{conj:gln}, and the identification of the full extremal simplex
...
\section{The geometric state space over Bruhat--Tits trees}\label{sec:geometry}
...
\begin{definition}[Lattices and the tree]\label{def:tree}
...
space; see Remark~\ref{rem:offbyone}.

\begin{definition}[Chain space]\label{def:chains}
Let
\[
...
Definition~\ref{def:boundary}. We call $\widetilde\Omega$ the \emph{Toeplitz} (or full)
state space and $\Omega$ the \emph{boundary} state space.
\end{definition}

\begin{remark}\label{rem:offbyone}
...
\begin{lemma}[Hecke recursion]\label{lem:recursion}
...
The pencil $1-Tu+qu^2$ of Lemma~\ref{lem:recursion} is the single object that reappears
in every section below: in Section~\ref{sec:thermo} as the inverse of a partition
function, in Section~\ref{sec:ktheory} as the presentation matrix of a $K$-group.

\begin{definition}[Hecke monoid and degree]\label{def:monoid}
...
\begin{definition}[Truncated Hecke groupoids]\label{def:groupoid}
...
\begin{equation}\label{eq:truncgroupoid}
...
Section~\ref{sec:thermo}: a measure class on $\Omega$ is a compatible family of
quasi-invariant classes for the finite-level relations $\cR^{(N)}$ below.

\begin{definition}[Tail equivalence]\label{def:tail}
...
\begin{proposition}\label{prop:groupoid}
...
\begin{remark}\label{rem:unitclass}
...
must be sought either in the global assembly (Section~\ref{sec:thermo}) or in the
quotient graph $Y$ (Section~\ref{sec:ktheory}). This is the structural reason the
...
\section{The thermodynamic--automorphic correspondence}\label{sec:thermo}
...
\begin{definition}[Satake potential and twisted dynamics]\label{def:dynamics}
...
Equation~\eqref{eq:RN} is the precise sense in which the Satake parameter enters the
...
\begin{theorem}[Partition function]\label{thm:partition}
...
Definition~\ref{def:dynamics}. Then the series converges absolutely if and only if
...
\begin{proof}
Substitute $u=q^{-\beta}$ in \eqref{eq:heckegen} of Lemma~\ref{lem:recursion}; the radius
...
the local factor of $\zeta(\beta)\zeta(\beta-1)$: Theorem~\ref{thm:partition} contains
...
\begin{theorem}[$\KMS$ classification, local]\label{thm:kms}
...
$e^{-\beta c_\pi}$ as in \eqref{eq:RN}. Then:
\begin{enumerate}
...
\begin{theorem}[Perron obstruction: states cannot see cuspidal parameters]\label{thm:perron}
Let $\sigma^{(\pi)}$ be as in Definition~\ref{def:dynamics}. A $\KMS_\beta$ state exists
...
function is $L_p(\beta,\pi_p)$ by Theorem~\ref{thm:partition};
...
\begin{remark}[Thermodynamic ghost states]\label{rem:ghost}
We propose the name \emph{thermodynamic ghost states} for the non-positive continuations
$\phi_{\beta,\pi}$ of Theorem~\ref{thm:perron}(i): they are $\sigma^{(\pi)}$-$\KMS$
...
the physical spectrum. The physical content of Theorem~\ref{thm:perron} is then this. The
...
Section~\ref{sec:ktheory} shows, as integral data in $K$-theory, where no positivity is
required.
\end{remark}

\begin{remark}
Theorem~\ref{thm:perron} is the operator-algebraic shadow of a familiar fact: positivity
...
\]
the product of the local conformal measures of Theorem~\ref{thm:kms}(ii), taken on the
...
\begin{theorem}[Kakutani dichotomy for Satake data]\label{thm:kakutani}
...
\end{equation}
the equivalence of the Hellinger criterion with \eqref{eq:obstruction} holding with
absolute implied constants depending only on the temperedness bound. The series
\eqref{eq:obstruction} is the exact $\GL_2$ analogue of the obstruction series
...
\begin{corollary}[Measure-theoretic strong multiplicity one]\label{cor:multone}
...
$\beta_c(S)=1$. Expanding \eqref{eq:obstruction} into Rankin--Selberg logarithms,
\[
...
\eqref{eq:obstruction} is a Hellinger distance \emph{between two} representations; it is
...
\begin{theorem}[Weight gap]\label{thm:gap}
...
By Theorem~\ref{thm:partition}, the equilibrium range is the domain of absolute
...
$|\widehat\lambda_p|\le1$, so the series \eqref{eq:obstruction} is bounded above
by $4\sum_{p\in S}p^{-\beta}$ and is therefore finite for every
$\beta>\beta_c(S)$. Disjointness and the width follow
\end{proof}

Theorem~\ref{thm:gap} is the main negative--positive result of the section. It says that
...
\begin{conjecture}[$\GL_n$ and ramified data]\label{conj:gln}
...
Definition~\ref{def:dynamics}, equals $L_p(\beta,\pi_p)=\prod_i(1-\alpha_{p,i}q^{-\beta})^{-1}$,
...
Theorems~\ref{thm:partition}--\ref{thm:gap}.
\end{conjecture}

\section{Algorithmic \texorpdfstring{$K$}{K}-theory via Hecke operators}\label{sec:ktheory}
...
\begin{definition}[Hecke correspondence]\label{def:corr}
...
\begin{theorem}[Hecke--Pimsner sequence; equivariant PV]\label{thm:pimsner}
...
\begin{equation}\label{eq:pimsnershort}
0\longrightarrow K_1(\cO_\cE)\longrightarrow\Z^n
\xrightarrow{\ (1-[\cE])^{t}\ }\Z^n\longrightarrow K_0(\cO_\cE)\longrightarrow0 .
\end{equation}
When $\cE$ is the correspondence of a single automorphism, \eqref{eq:pimsnershort} is the
...
\begin{definition}[Ihara module]\label{def:ihara}
The \emph{Ihara module} of $(\Gamma,p)$ is the $\Z[u^{\pm1}]$-module
\[
M_\Gamma(p):=\Z[u^{\pm1}]^{\,n}\big/\bigl(1-u\,T_p+q\,u^2\bigr)\Z[u^{\pm1}]^{\,n},
\]
the presentation matrix being the Hecke pencil of Lemma~\ref{lem:recursion}.
\end{definition}

\begin{proposition}[Bass's identity; the two channels]\label{prop:bass}
With $A^{\mathrm{nb}}_Y$ as in Definition~\ref{def:groupoid},
...
\eqref{eq:bass} has been verified symbolically for all graphs of
Table~\ref{tab:snf}. Specialising $u=1$:
\[
\bigl(1-uT_p+qu^2\bigr)\big|_{u=1}=(q+1)I_n-T_p=\Delta_p .
\]
\end{proposition}

Proposition~\ref{prop:bass} isolates the two channels available for computation. The
...
is the $u=1$ specialisation of the Ihara module. They are related by \eqref{eq:bass},
and --- this is the point of the section --- they carry completely different arithmetic.

\subsection{The structure theorem}

\begin{theorem}[Hecke--Laplacian structure theorem]\label{thm:structure}
...
\]
of the Ihara module of Definition~\ref{def:ihara}, and they are the integral index
...
Theorem~\ref{thm:pimsner} applied with index map $\Delta_p$. In (iii), Kirchhoff gives
...
\begin{remark}[Bulk and boundary: what is, and is not, an algebra]\label{rem:noalgebra}
...
\end{tabular}
\end{center}
Everything asserted in Theorem~\ref{thm:structure} is unconditional at the level of the
...
lives. The Ihara pencil $1-uT_p+qu^2$ of Definition~\ref{def:ihara} is a quadratic
...
\begin{theorem}[Boundary channel is arithmetically blind]\label{thm:blind}
...
to compute $\coker M=\Z^{E_Y}/M\Z^{E_Y}$, and by Theorem~\ref{thm:pimsner} this is
...
\begin{equation}\label{eq:blindrel}
[f]+[\bar f]=c_{o(f)},\qquad\text{where}\quad c_v:=\sum_{t(e)=v}[e].
\end{equation}
Applying \eqref{eq:blindrel} to $f$ and to $\bar f$ gives $c_{o(f)}=c_{t(f)}$ for every
...
\begin{remark}[Where the spectrum goes]\label{rem:wherespectrum}
The proof shows exactly how the Hecke data disappear, and it is worth recording that the
mechanism is \emph{not} the one suggested by Bass's identity \eqref{eq:bass}: at $u=1$
both factors of \eqref{eq:bass} vanish, so \eqref{eq:bass} degenerates to $0=0$ and
carries no integral information there. What kills the spectrum is the local relation
\eqref{eq:blindrel}: it forces the $n$ a priori distinct classes $c_v$ to collapse to a
...
Theorems~\ref{thm:structure} and \ref{thm:blind} together are the main structural finding
...
\begin{corollary}[Frobenius traces as torsion]\label{cor:frobenius}
...
\begin{corollary}[Ramanujan bound; strict, and never attained]\label{cor:ramanujan}
...
and the deficit is exponential in $n$: see Theorem~\ref{thm:bstorsion} and
Remark~\ref{rem:bsnumerics}.
\end{corollary}

\begin{theorem}[Benjamini--Schramm asymptotic torsion]\label{thm:bstorsion}
...
Theorem~\ref{thm:structure}. Then the torsion density converges, and its limit is Lyons'
tree entropy of $T_{q+1}$:
\begin{equation}\label{eq:bslimit}
...
\begin{corollary}[The deficit, to two orders]\label{cor:deficit}
As $q\to\infty$,
...
=2q^{-1/2}-\tfrac32q^{-1}+O(q^{-3/2})$, which is \eqref{eq:deficit}. (A direct
...
numerical values of Remark~\ref{rem:bsnumerics} and of the ratios above were computed
independently of this expansion and agree with it: at $q=10^4$, \eqref{eq:deficit}
predicts $0.02-0.00015=0.01985$ against the quadrature value $0.019852$.
\end{proof}

\begin{proof}[Proof]
By Theorem~\ref{thm:structure}(iii), $|\Tors K_0(\cO_j)|=\kappa(Y_j)$, so
\eqref{eq:bslimit} is Lyons' theorem on the asymptotics of the number of spanning trees
...
\begin{remark}[Why the Ramanujan bound is the wrong instrument]\label{rem:bsnumerics}
Corollary~\ref{cor:ramanujan} replaces the whole spectrum by its extreme point
...
Theorem~\ref{thm:bstorsion}.

The deeper point is that the Ramanujan property is neither necessary nor sufficient for
the limit \eqref{eq:bslimit}, and that the invariant which does control the torsion
...
One further datum belongs here, because it bears on Remark~\ref{rem:ghost} and on the
...
\begin{algorithm}[Hecke $\to$ torsion]\label{alg:main}
...
must agree. Optionally verify Bass's identity \eqref{eq:bass} symbolically as a check on
the graph data.
\end{enumerate}
...
Theorem~\ref{thm:structure}; the order always equals
$\frac1n\prod_{\lambda\neq q+1}(q+1-\lambda)$. Columns 6--7: the boundary channel,
Theorem~\ref{thm:blind}; the torsion is always cyclic of order $r-1=(q-1)n/2$,
independently of the spectrum, confirming the proof given there. Bass's identity
\eqref{eq:bass} was verified symbolically in the first five rows.}
\label{tab:snf}
\end{table}

\begin{example}[Level $37$, $p=2$: two elliptic curves, one torsion group]\label{ex:37}
...
eigenvalues $5$ and $3$, so \eqref{eq:torsionorder} gives
...
\begin{example}[Level $11$, $p=3$]\label{ex:11}
...
\begin{remark}[Weighted quotients]\label{rem:weights}
Examples~\ref{ex:37} and \ref{ex:11} are stated at the level of the eigenvalue identity
$q+1-a_p=\#E(\F_p)$, which is unconditional. The passage to the critical group in
Theorem~\ref{thm:structure}(iii) requires $\Gamma$ torsion-free, so that $Y$ is a genuine
...
Theorem~\ref{thm:structure} holds in its weighted form, with $\Delta_p$ the weighted
...
$\tfrac12(4-a_3)$ of Example~\ref{ex:11} is not an integer --- become integral once read
in the equivariant group of Proposition~\ref{prop:equivariant} rather than in $\Z$. The
...
\begin{proposition}[Equivariant Hecke $K$-theory]\label{prop:equivariant}
...
Theorem~\ref{thm:pimsner} is the $R(C)$-linear extension of $\Delta_p$, and
\[
...
& Radon--Nikodym cocycle $e^{-\beta c_\pi}$ & Def.~\ref{def:dynamics}\\
local $L$-factor $L_p(\beta,\pi_p)$
& partition function $Z_p(\beta;\pi)$ & Thm~\ref{thm:partition}\\
largest pole of $L_p$
& critical inverse temperature $\beta_c(\pi_p)$ & Thm~\ref{thm:partition}\\
temperedness (Ramanujan) at $p$
& $\beta_c(\pi_p)=w/2$ & Thm~\ref{thm:partition}\\
Eisenstein locus
& the only locus carrying $\KMS$ \emph{states} & Thm~\ref{thm:perron}\\
Rankin--Selberg $L(s,\pi\times\widetilde{\pi'})$
& Kakutani/Hellinger obstruction $\Xi_\beta$ & Thm~\ref{thm:kakutani}\\
strong multiplicity one
& mutual singularity of phases & Cor.~\ref{cor:multone}\\
motivic weight $w$
& width of the equilibrium/obstruction gap & Thm~\ref{thm:gap}\\
Hecke recursion $T T_n=T_{n+1}+qT_{n-1}$
& Ihara module $\Z[u^{\pm}]^n/(1-uT_p+qu^2)$ & Def.~\ref{def:ihara}\\
Ihara zeta $Z_Y(u)$
& Fitting ideal of the Ihara module & Prop.~\ref{prop:bass}\\
Frobenius trace $a_p(f)$, $\#E_f(\F_p)$
& invariant factors of $\SNF(\Delta_p)$ & Thm~\ref{thm:structure}\\
Euler characteristic of $Y$ (no arithmetic)
& torsion of the boundary quotient & Thm~\ref{thm:blind}\\
\bottomrule
\end{tabular}
\end{center}
...
field theory and Artin data, and Theorem~\ref{thm:gap} measures precisely how far it
...
$\GL_2$ is therefore not one bridge but two, and Bass's identity \eqref{eq:bass} is the
...
computations of Table~\ref{tab:snf}, and the spectral and torsion-density computations of
Remark~\ref{rem:bsnumerics}, are exact or numerically controlled and were carried out
independently of the statements they confirm. Theorem~\ref{thm:bstorsion} rests on
Lyons' spanning-tree asymptotics \cite{Lyons2005}, which we quote and do not prove, and
Remark~\ref{rem:bsnumerics} on the Kesten--McKay spectral distribution
\cite{Kesten1959,McKay1981}. The one substantive gap left open is the one isolated in
Remark~\ref{rem:noalgebra}: the Laplacian channel is a module and an index, not yet a
$C^*$-algebra.

\begin{thebibliography}{9}
\bibitem{Lyons2005} R.~Lyons, \emph{Asymptotic enumeration of spanning trees},
Combin.\ Probab.\ Comput.\ \textbf{14} (2005), no.~4, 491--522. [Primary-source
verified: Cambridge UP, DOI 10.1017/S096354830500684X.]
\bibitem{Kesten1959} H.~Kesten, \emph{Symmetric random walks on groups},
Trans.\ Amer.\ Math.\ Soc.\ \textbf{92} (1959), no.~2, 336--354. [Crossref-verified:
DOI 10.1090/S0002-9947-1959-0109367-6.]
\bibitem{McKay1981} B.~D.~McKay, \emph{The expected eigenvalue distribution of a large
regular graph}, Linear Algebra Appl.\ \textbf{40} (1981), 203--216.
[Crossref-verified: DOI 10.1016/0024-3795(81)90150-6.]
\end{thebibliography}
