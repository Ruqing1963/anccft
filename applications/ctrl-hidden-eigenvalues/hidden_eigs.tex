...
\title{\bfseries Power-sum lower bounds on hidden eigenvalues\\ in positive realizations}
...
sum of the poles and $[x]_+=\max(x,0)$. Theorem~\ref{thm:main} states
\begin{equation}\label{eq:bound}
s\;\ge\;\Bigl\lceil\;\max_{k\ge1}\;\frac{[-p_k]_+}{\rho^{k}}\;\Bigr\rceil .
\end{equation}

\paragraph{What is already known.} The case $k=1$ of \eqref{eq:bound} is published.
...
Nagy and Matolcsi \cite{NM03}, not in \cite{BF04,TS25}. Section~\ref{sec:improve} shows
...
Since $p_k\ge-\sum_i|\lambda_i|^{k}\ge-n\rho^{k}$, the right-hand side of \eqref{eq:bound}
never exceeds $n$, so \eqref{eq:bound} can never prove more than $N\le 2n$
(Proposition~\ref{prop:ceiling}). The bound is therefore complementary to the combinatorial
...
\begin{proposition}[$k=1$ is Benvenuti's $\zeta$]\label{prop:k1}
At $k=1$, \eqref{eq:bound} reads $s\ge\lceil-p_1(\Lambda)/\rho\rceil$, which after
...
\begin{proposition}[Ceiling of the method]\label{prop:ceiling}
The right-hand side of \eqref{eq:bound} is at most $n$; consequently
Theorem~\ref{thm:main} can never certify $N>2n$.
\end{proposition}
...
\section{Strict improvement, and an exact determination}\label{sec:improve}
...
\item[(ii)] Theorem~\ref{thm:main} at $k=2$ gives
...
Table~\ref{tab:improve} shows the three quantities. The criterion $\theta^{2}>1-1/2m$ of
Theorem~\ref{thm:improve}(iv) predicts exactly which rows close: it holds for
...
\toprule
$\theta$ & $m$ & $n$ & \cite[Thm.~4.7]{Ben20ELA} & Theorem~\ref{thm:main} & \cite[Thm.~4.4]{Ben20ELA}\\
 & & & lower & lower & upper\\
\midrule
...
$p_2(\M_m)=1-2m$, so Theorem~\ref{thm:main} gives $s\ge2m-1$ and $N\ge4m$. The bound is
...
than through traces; we note this so that Theorem~\ref{thm:sharp} is not mistaken for a new
determination. It is a sharpness check on \eqref{eq:bound}, not a new value of $N$. Second,
$4m=2n-2$ is the Benvenuti--Farina ratio of \cite{BF99}, as recalled in
Section~\ref{sec:intro}.
\end{remark}

\begin{remark}[Tutorial Example 8]\label{rem:ex8}
For the poles $\{1,\pm0.9i\}$ of \cite[Ex.~8]{BF04}, $p_2=-31/50$ and \eqref{eq:bound}
...
\section{Why there is no analogue for the Boyle--Handelman problem}\label{sec:bh}
...
obstruction, not a counting device, and no lower bound of the shape \eqref{eq:bound} on the
size of the dilation can exist. Equivalently: in that category the analogue of
Theorem~\ref{thm:main} is the statement $p_k(\Lambda)\ge0$, with no $s$ in it.
...
Table~\ref{tab:battery} in $23$\,s, all passing; its output is archived as
...
\begin{table}[ht]
\centering\small
\begin{tabular}{cp{10.4cm}c}
\toprule
key & check & mode\\
\midrule
(T) & Theorem~\ref{thm:main} holds on $120$ random nonnegative integer matrices, $\Lambda$ a rational factor & exact $+$ numeric $\rho$\\
(K) & at $k=1$ the bound equals $\zeta$ of \cite[Thm.~4.7]{Ben20ELA} & exact\\
(I) & $k=2$ strictly exceeds \cite[Thm.~4.7]{Ben20ELA} in $15/15$ instances (Table~\ref{tab:improve}) & exact\\
(S) & Theorem~\ref{thm:sharp}: bound $=4m$ attained by $C_4^{\oplus m}$, $m\le32$ & exact\\
(E) & Remark~\ref{rem:ex8}: bound $=1$, attained by an explicit rational circulant & exact\\
(C) & Proposition~\ref{prop:ceiling}: the bound never exceeds $n$, on $300$ random pole sets & exact\\
(Z) & appending zeros leaves every power sum unchanged (Section~\ref{sec:bh}) & exact\\
\bottomrule
\end{tabular}
...
\emph{Established here.} Theorem~\ref{thm:main} for all $k$; its coincidence with
...
orders, give a bound not implied by \eqref{eq:bound}; we have not investigated this and it
is the obvious next step. (c) Whether the bound is ever tight outside the families of
Sections~\ref{sec:improve}--\ref{sec:sharp}; it was not tight in any of our $120$ random
instances.
...
Unrefereed preprint. Theorem~\ref{thm:main} is elementary and its proof is complete above.
The bounds of \cite{Ben20ELA} quoted for comparison in Table~\ref{tab:improve} are
...
Section~\ref{sec:bh} are quoted from \cite{LM95,BH91,KOR00}. We searched the literature for
...
\bibitem{ADFB96} B.~D.~O.~Anderson, M.~Deistler, L.~Farina and L.~Benvenuti,
\emph{Nonnegative realization of a linear system with nonnegative impulse response},
IEEE Trans.\ Circuits Syst.\ I \textbf{43} (1996), 134--142.

\bibitem{Ben20ELA} L.~Benvenuti,
\emph{The NIEP and the positive realization problem},
Electron.\ J.\ Linear Algebra \textbf{36} (2020), 367--384.

\bibitem{Ben20SCL} L.~Benvenuti,
\emph{A lower bound on the dimension of minimal positive realizations for discrete time
systems}, Systems Control Lett.\ \textbf{135} (2020), 104595.

\bibitem{BF99} L.~Benvenuti and L.~Farina,
\emph{An example of how positivity may force realizations of ``large'' dimension},
Systems Control Lett.\ \textbf{36} (1999), 261--266.

\bibitem{BF04} L.~Benvenuti and L.~Farina,
\emph{A tutorial on the positive realization problem},
IEEE Trans.\ Automat.\ Control \textbf{49} (2004), 651--664.

\bibitem{BH91} M.~Boyle and D.~Handelman,
\emph{The spectra of nonnegative matrices via symbolic dynamics},
Ann.\ of Math.\ \textbf{133} (1991), 249--316.

\bibitem{ANCFT} R.~Chen,
\emph{Algorithmic non-commutative class field theory, I--XVI}, preprints (2026);
here II (the trace obstruction under shift equivalence), X and XI.

\bibitem{Farina96} L.~Farina,
\emph{On the existence of a positive realization},
Systems Control Lett.\ \textbf{28} (1996), 219--226.

\bibitem{KOR00} K.~H.~Kim, N.~S.~Ormes and F.~W.~Roush,
\emph{The spectra of nonnegative integer matrices via formal power series},
J.\ Amer.\ Math.\ Soc.\ \textbf{13} (2000), 773--806.

\bibitem{LM95} D.~Lind and B.~Marcus,
\emph{An Introduction to Symbolic Dynamics and Coding},
Cambridge University Press, 1995.

\bibitem{LoewyLondon78} R.~Loewy and D.~London,
\emph{A note on an inverse problem for nonnegative matrices},
Linear Multilinear Algebra \textbf{6} (1978), 83--90.

\bibitem{NM03} B.~Nagy and M.~Matolcsi,
\emph{A lowerbound on the dimension of positive realizations},
IEEE Trans.\ Circuits Syst.\ I \textbf{50} (2003), 782--784.

\bibitem{OMK84} Y.~Ohta, H.~Maeda and S.~Kodama,
\emph{Reachability, observability and realizability of continuous-time positive systems},
SIAM J.\ Control Optim.\ \textbf{22} (1984), 171--180.

\bibitem{TS25} H.~Taghavian and J.~Sj\"olund,
\emph{Minimal positive Markov realizations},
preprint (2025), arXiv:2502.21102.
