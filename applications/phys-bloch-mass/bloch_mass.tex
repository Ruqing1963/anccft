...
% Build: pdflatex bloch_mass (x3)
\documentclass[aps,prb,reprint,amsmath,amssymb,floatfix,superscriptaddress]{revtex4-2}
...
\newcommand{\Yt}{\widetilde Y_{\bm\alpha}}

\begin{document}

\title{Effective mass from spanning trees, and the absence of Bloch phases\\
on rank-two hyperbolic lattices}

\author{Ruqing Chen}
\email{ruqing@hotmail.com}
\affiliation{GUT Geoservice Inc., Rivi\`ere-Beaudette, Qu\'ebec J0P 1R0, Canada}
...
content of Secs.~\ref{sec:mass}--\ref{sec:trees}, whose sharpest consequence is a
quantization statement (Corollary~\ref{cor:quant}): the entries of $M^{-1}$, measured in the
...
are the Ramanujan complexes~\cite{LSV}. These are the higher-rank hyperbolic lattices. We
show in Sec.~\ref{sec:nogo} that for them band theory does not merely become harder, it
...
(Table~\ref{tab:trees}). It is also exactly the vertex link of a thick $\Atwo$ building of
order two. Assembling Heawood links into a rank-two complex destroys $b_1$ completely
(Table~\ref{tab:ranktwo}). The obstruction is not local.

\subsection*{Relation to previous work}

Theorem~\ref{thm:mass} lies close to known mathematics. Once the Albanese metric is
identified with the covariance of the crystal-lattice central limit theorem~\cite{KS1} and
with a Kirchhoff ratio~\cite{KS3,BF}, Eq.~\eqref{eq:band} follows from a second-order
perturbation argument, which we give in full for completeness rather than for novelty. What
we claim as new is: the tensor form of Theorem~\ref{thm:mass} together with the explicit
statement that this object is the physical effective-mass tensor; the quantization
Corollary~\ref{cor:quant} and its reading as a rigid constraint on measurable band
curvature; the deletion--contraction closed form \eqref{eq:mstar}; the loop-flux
Corollary~\ref{cor:loop}; and the whole of Sec.~\ref{sec:nogo}. The closest prior result in
...
For Sec.~\ref{sec:nogo} we have found no prior work: we are not aware of any physics
...
\section{Lattices from abelian covers}\label{sec:setup}
...
\begin{definition}[The lattice]\label{def:lattice}
...
\section{The effective-mass tensor is the Albanese metric}\label{sec:mass}
...
\begin{theorem}[Effective mass]\label{thm:mass}
Under the hypothesis of Definition~\ref{def:lattice}, $\lambda_0$ is even in $\bm k$,
...
\begin{equation}\label{eq:albanese}
...
\eqref{eq:bloch} about $\bm k=0$ gives $\Delta_{\bm k}=\Delta_0+\sum_a k_aD^{(1)}_a
...
\eqref{eq:albanese}, $h_a$ is the orthogonal projection of $\alpha_a$ onto the column space
...
theorem $\prod_{j\ge1}\lambda_j(0)=n\kappa_0$, which with \eqref{eq:band} gives
\eqref{eq:floquet}.
\end{proof}

For $d=1$ Eq.~\eqref{eq:band} is the effective-mass identity of
...
\eqref{eq:albanese} for $d=1$ --- that the Albanese metric evaluates the harmonic energy ---
...
Secs.~\ref{sec:trees}--\ref{sec:einstein}.

\begin{remark}
Equation~\eqref{eq:floquet} is the practically useful form: $\Theta(\bm k)$ is a Laurent
...
how the battery of Sec.~\ref{sec:battery} verifies Theorem~\ref{thm:mass} in exact
arithmetic.
\end{remark}

\section{Spanning trees and the quantization of effective mass}\label{sec:trees}

\begin{theorem}[Single-link flux]\label{thm:trees}
...
$\kappa_0=\tau(Y_0-e)+\tau(Y_0/e)$ finishes it. Equation~\eqref{eq:mstar} is
\eqref{eq:band}.
\end{proof}

\begin{corollary}[Quantization of effective mass]\label{cor:quant}
...
$\kappa_0H_{ab}=\bm a_a^{t}(\kappa_0G^{-1})\bm a_b\in\Z$ by \eqref{eq:albanese}.
\end{proof}
...
$2t/n$, it is a rational with denominator dividing $\kappa_0$. Table~\ref{tab:trees}
lists the values for six unit cells.

The algebra behind Corollary~\ref{cor:quant} is elementary, and in the equivalent form
...
Eq.~\eqref{eq:mstar}. For the Heawood graph $\kappa_0=3\cdot7^{5}$ and
$\tau(Y_0-e)=8\cdot7^{4}$. All entries exact.}
\label{tab:trees}
\end{table}

\begin{corollary}[Loop flux]\label{cor:loop}
...
Corollary~\ref{cor:loop} is the sharp opposite of Theorem~\ref{thm:trees}: a single link is
maximally sensitive to the ambient cell (through $\Reff$), a closed loop is completely
insensitive to it. Both are verified in Sec.~\ref{sec:battery} on $K_5$, where the
...
(Corollary~\ref{cor:quant}); the off-diagonal entries are the mass anisotropy. All entries
exact.}
\label{tab:tensor}
\end{table}

\section{An Einstein relation}\label{sec:einstein}
...
\begin{theorem}[Diffusion tensor]\label{thm:einstein}
...
$H=(n/2t)M^{-1}$ and $m=n(q{+}1)/2$ gives \eqref{eq:einstein}.
\end{proof}

Equation~\eqref{eq:einstein} says that the quantum band curvature and the classical
...
to scale, and both are quantized by Corollary~\ref{cor:quant}. The $d=1$ case is the
crystal-lattice central limit theorem of Kotani and Sunada~\cite{KS1}, in which the
covariance is the Albanese metric; Theorem~\ref{thm:mass} is what identifies that covariance
with a band curvature.

\section{Rank two: no Brillouin zone}\label{sec:nogo}
...
plaquettes; the Ramanujan complexes of Lubotzky, Samuels and Vishne~\cite{LSV} are among
...
\begin{theorem}[No band structure in rank two]\label{thm:nogo}
...
exact ($p$-adic Smith normal forms; see Sec.~\ref{sec:battery}).}
\label{tab:ranktwo}
\end{table}

Table~\ref{tab:ranktwo} makes the statement concrete. On $Y_{21}$ the flux group is
...
\begin{remark}[What this does \emph{not} say]\label{rem:notsay}
Theorem~\ref{thm:nogo} is a statement about translational symmetry, not about spectra. The
...
Theorem~\ref{thm:nogo}(iv) not even an infinitesimal one is available near the trivial
character. The effective mass of Sec.~\ref{sec:mass} is simply not defined there: it is
the curvature of a band that does not exist.
\end{remark}

\begin{remark}[The obstruction is global]\label{rem:global}
...
$tM^{*}=147/8$ (Table~\ref{tab:trees}). As the \emph{link} of a rank-two complex it is part
of a structure with $b_1=0$ (Table~\ref{tab:ranktwo}). No local inspection of a rank-two
...
\section{Numerical verification}\label{sec:battery}
...
key & check & $\#$ & mode\\
\colrule
(S) & $H=1-\Reff(e)$ and $\kappa_0H=\tau(Y_0-e)$, Eq.~\eqref{eq:trees} & $6/6$ & exact\\
(M) & $\partial^2_{ab}\Theta|_0=2\kappa_0H$, Eq.~\eqref{eq:floquet} & $15/15$ & exact\\
(M) & $\lambda_0(\bm k)=\bm k^{t}H\bm k/n+O(|\bm k|^{4})$ & $15/15$ & numeric\\
(Q) & $\kappa_0H$ integral, Eq.~\eqref{eq:quant} & $15/15$ & exact\\
(E) & $\sigma^{2}=H/m$ by Green--Kubo, Eq.~\eqref{eq:einstein} & $15/15$ & exact\\
(G) & gap $\lambda_1(0)>0$ and $\lambda_0(0)=0$ & $15/15$ & numeric\\
(X) & $H$ reproduces Refs.~\onlinecite{ANCFT9,ANCFT14} & $4/4$ & exact\\
(C) & loop flux gives $H=\ell$, Corollary~\ref{cor:loop} & $1/1$ & exact\\
(B) & $b_1=0$ and $H_1(Y;\Z)$, Table~\ref{tab:ranktwo} & $4/4$ & exact\\
\colrule
 & total & $90/90$ & \\
\end{tabular}
\end{ruledtabular}
...
A word on the two \emph{numeric} rows. The band identity \eqref{eq:band} is checked by
...
Floquet-determinant form \eqref{eq:floquet} by interpolating $\Theta$ as a Laurent
...
Ref.~\onlinecite{ANCFT10}: the complexes of Table~\ref{tab:ranktwo} are abelian covers of
...
Corollary~\ref{cor:quant} is a constraint on a measurable quantity. For a synthetic lattice
...
Theorem~\ref{thm:trees} says more: for a single flux link, the mass is set by the ratio of
two spanning-tree counts, so it can be engineered by choosing where the flux link sits, with
no computation beyond counting trees. Corollary~\ref{cor:loop} gives the complementary
...
Equation~\eqref{eq:einstein} is a consistency check available without any quantum
...
character variety of a non-abelian surface group. Theorem~\ref{thm:mass} is a statement
...
(c) Rank two without a Brillouin zone. Theorem~\ref{thm:nogo} removes the tool but not the
...
for any $d\ge1$ and any $q\ge2$ (Theorem~\ref{thm:nogo}). Any construction claiming a
...
Sec.~\ref{sec:intro}, that no one appears to have asked the question, does.
...
crystal-lattice central limit theorem; Secs.~\ref{sec:mass}--\ref{sec:einstein} are a
physical reading of that material and not a replacement for it.

Attribution, statement by statement. The $d=1$ case of Theorem~\ref{thm:mass} is
Ref.~\onlinecite{ANCFT14}, Theorem~4.2; Eq.~\eqref{eq:trees} for $d=1$ is
Ref.~\onlinecite{ANCFT9}, Theorem~1.2(iv); Theorem~\ref{thm:nogo}(i) is
...
substance of Corollary~\ref{cor:quant}, is in Refs.~\onlinecite{KS3,BF}. What the present
paper adds is the $d>1$ tensor form, the identification of that tensor with $M^{-1}$, the
deletion--contraction form \eqref{eq:mstar}, Corollary~\ref{cor:loop}, the Einstein relation
in the form \eqref{eq:einstein}, and Sec.~\ref{sec:nogo} --- the reading of the rank-two
...
The complex builder and the exact $p$-adic Smith normal form used in Sec.~\ref{sec:battery}
are those of Ref.~\onlinecite{ANCFT10}.
\end{acknowledgments}

\begin{thebibliography}{99}

\bibitem{KFH} A.~J.~Koll\'ar, M.~Fitzpatrick, and A.~A.~Houck,
\emph{Hyperbolic lattices in circuit quantum electrodynamics},
Nature \textbf{571}, 45 (2019).

\bibitem{MR1} J.~Maciejko and S.~Rayan,
\emph{Hyperbolic band theory},
Sci.\ Adv.\ \textbf{7}, eabe9170 (2021).

\bibitem{MR2} J.~Maciejko and S.~Rayan,
\emph{Automorphic Bloch theorems for hyperbolic lattices},
Proc.\ Natl.\ Acad.\ Sci.\ USA \textbf{119}, e2116869119 (2022).

\bibitem{Leng22} P.~M.~Lenggenhager \emph{et al.},
\emph{Simulating hyperbolic space on a circuit board},
Nat.\ Commun.\ \textbf{13}, 4373 (2022).

\bibitem{Huang24} L.~Huang \emph{et al.},
\emph{Hyperbolic photonic topological insulators},
Nat.\ Commun.\ \textbf{15}, 1647 (2024).

\bibitem{Xu25} X.~Xu, A.~A.~Mahmoud, N.~Gorgichuk, R.~Thomale, S.~Rayan, and M.~Mariantoni,
\emph{A scalable superconducting circuit framework for emulating physics in hyperbolic
space}, arXiv:2510.23827 (2025).

\bibitem{KS1} M.~Kotani and T.~Sunada,
\emph{Albanese maps and off diagonal long time asymptotics for the heat kernel},
Commun.\ Math.\ Phys.\ \textbf{209}, 633 (2000).

\bibitem{KS2} M.~Kotani and T.~Sunada,
\emph{Standard realizations of crystal lattices via harmonic maps},
Trans.\ Am.\ Math.\ Soc.\ \textbf{353}, 1 (2001).

\bibitem{KS3} M.~Kotani and T.~Sunada,
\emph{Jacobian tori associated with a finite graph and its abelian covering graphs},
Adv.\ Appl.\ Math.\ \textbf{24}, 89 (2000).

\bibitem{BF} M.~Baker and X.~Faber,
\emph{Metric properties of the tropical Abel--Jacobi map},
J.\ Algebr.\ Comb.\ \textbf{33}, 349 (2011).

\bibitem{KorS} E.~Korotyaev and N.~Saburova,
\emph{Effective masses for Laplacians on periodic graphs},
J.\ Math.\ Anal.\ Appl.\ \textbf{436}, 104 (2016).

\bibitem{Ga} H.~Garland,
\emph{$p$-adic curvature and the cohomology of discrete subgroups of $p$-adic groups},
Ann.\ Math.\ \textbf{97}, 375 (1973).

\bibitem{BSw} W.~Ballmann and J.~\'Swi\k{a}tkowski,
\emph{On $L^{2}$-cohomology and property (T) for automorphism groups of polyhedral cell
complexes},
Geom.\ Funct.\ Anal.\ \textbf{7}, 615 (1997).

\bibitem{LSV} A.~Lubotzky, B.~Samuels, and U.~Vishne,
\emph{Ramanujan complexes of type $\widetilde A_d$},
Isr.\ J.\ Math.\ \textbf{149}, 267 (2005).

\bibitem{CMSZ} D.~I.~Cartwright, A.~M.~Mantero, T.~Steger, and A.~Zappa,
\emph{Groups acting simply transitively on the vertices of a building of type
$\widetilde A_2$, I},
Geom.\ Dedicata \textbf{47}, 143 (1993).

\bibitem{ANCFT9} R.~Chen,
\emph{Algorithmic non-commutative class field theory, IX: exceptional zeros, the graph
$\mathcal L$-invariant, and the operator-algebraic Mazur--Tate--Teitelbaum formula},
preprint (2026).

\bibitem{ANCFT10} R.~Chen,
\emph{Algorithmic non-commutative class field theory, X: $\widetilde A_2$ complexes, higher
simplicial Laplacians, and the fate of the trace obstruction in rank two},
preprint (2026).

\bibitem{ANCFT12} R.~Chen,
\emph{Algorithmic non-commutative class field theory, XII: $\widetilde A_2$ towers, no
abelian sector, the covering norm theorem, and the two digraph towers},
preprint (2026).

\bibitem{ANCFT14} R.~Chen,
\emph{Algorithmic non-commutative class field theory, XIV: the graph $\mathcal L$-invariant
as spectral action, Bloch effective mass and Albanese length, and what the cyclic Chern
character sees},
preprint (2026).
