% generated from Bio 7/rc_quotient_signed.tex
\chapter{The reverse-complement quotient is a signed graph: what Zaslavsky's theory gives, and what it cannot}\label{chap:Bio7}
\chaptermark{Bio7}
...
and lives on the digraph, and Theorem~\ref{Bio7:thm:anti} says the digraph admits no such theory.
Separately, Theorem~\ref{Bio7:thm:series} says the chain contraction that reduces a bacterial
...
\begin{proposition}\label{Bio7:prop:parity}
...
\label{Bio7:tab:parity}
\end{table}

Table~\ref{Bio7:tab:parity} makes the point concretely and shows that the obstruction is not
...
\begin{remark}[What the fixed points are, topologically]\label{Bio7:rem:halfedge}
...
exactly why the quotient adjacency matrix in Table~\ref{Bio7:tab:parity} fails to be symmetric:
...
\begin{definition}\label{Bio7:def:sigma}
...
Two facts make Definition~\ref{Bio7:def:sigma} usable.

\begin{lemma}\label{Bio7:lem:mono}
...
\begin{lemma}\label{Bio7:lem:switch}
...
\Delta_{\Sig_k}$, and with it the critical group of Section~\ref{Bio7:sec:rt}, depends only on
$(S,k)$.
\end{lemma}
...
\begin{theorem}\label{Bio7:thm:balance}
...
Definition~\ref{Bio7:def:sigma}. The companion note proves that $D_k$ is connected exactly when
...
Theorem~\ref{Bio7:thm:balance} is worth stating because it converts a purely combinatorial
...
\section{The covering theory that does exist}\label{Bio7:sec:rt}
...
\begin{theorem}[Reiner--Tseng \cite{RT14}, Thm.~1.2, verified here]\label{Bio7:thm:rt}
...
Theorem~\ref{Bio7:thm:rt} is a statement about $|D_k|$, the undirected shadow. The quantity the
...
\begin{theorem}\label{Bio7:thm:anti}
...
\begin{remark}[What $\rho$ gives instead: a symmetric linking form]\label{Bio7:rem:pairing}
...
with finite cokernel, and Theorem~\ref{Bio7:thm:anti} identifies $\Delta^{\mathsf T}$ with
...
\section{Signed bundle chains do not contract}\label{Bio7:sec:series}
...
\begin{theorem}\label{Bio7:thm:series}
...
Theorem~\ref{Bio7:thm:series}(a) is visible --- but the order ratio never equals $r^s$, and is
not even an integer in five rows. Check (C).}
\label{Bio7:tab:series}
\end{table}

Table~\ref{Bio7:tab:series} separates the two halves of the theorem. What survives contraction is
...
\begin{corollary}\label{Bio7:cor:nocompute}
...
Corollary~\ref{Bio7:cor:nocompute} says $\K(\Sig_k)$ cannot be computed by contraction at this
size, and Theorem~\ref{Bio7:thm:anti} says that even if it could, it would not count
...
undirected shadow. Proposition~\ref{Bio7:prop:parity}, Lemma~\ref{Bio7:lem:mono},
Lemma~\ref{Bio7:lem:switch}, Theorem~\ref{Bio7:thm:anti} and Theorem~\ref{Bio7:thm:series} are proved in
full generality. Theorem~\ref{Bio7:thm:balance} is a combination of Reiner--Tseng
Prop.~11.4--11.5 with the connectivity criterion of the companion note. Theorem~\ref{Bio7:thm:rt}
...
extension arising from a real genome. Part (b) of Theorem~\ref{Bio7:thm:series} --- that the
...
Section~\ref{Bio7:sec:rt} are Reiner and Tseng's \cite{RT14}; the competing rooted convention and
...
Theorem~\ref{Bio7:thm:anti} shows is unavailable here, is Bilu and Linial's \cite{BL06}. The
...
\emph{First}, Theorem~\ref{Bio7:thm:series}(b) in general: prove that a signed $r$-bundle chain of
...
\emph{Third}, and most important, Theorem~\ref{Bio7:thm:anti} closes the quotient route to a
double-stranded reconstruction count but does not close the problem. What survives is the
symmetric linking form of Remark~\ref{Bio7:rem:pairing}, and that is the structure a correct
