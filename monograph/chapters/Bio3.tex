...
$\K(G_k)$ the sandpile group. The first companion paper [Ch.~\ref{chap:Bio1}] showed that $\K(G_k)$
...
paper [Ch.~\ref{chap:Bio2}] used the group as a coordinate system on reconstructions and found that
...
sandpile group unchanged [Ch.~\ref{chap:Bio1}, Lem.~4]; this is the $r=1$ case of
...
$\K(G)\cong(\Z/r)^{s-1}$; this is the single-repeat theorem of [Ch.~\ref{chap:Bio1}, Thm.~5].

\begin{remark}\label{Bio3:rem:r1}
For $r=1$ the summand is trivial and the lemma says that smoothing a vertex of in- and
out-degree one preserves the group, which is [Ch.~\ref{chap:Bio1}, Lem.~4]. The content of the
...
multiplicities $r_i$, and suppose the \emph{clean} hypothesis of [Ch.~\ref{chap:Bio1}]: the $k$-mers
...
For $m=1$, $D_w$ is a single vertex and $\K=(\Z/r)^{s-1}$ [Ch.~\ref{chap:Bio1}, Thm.~5]. For $m=2$,
$r_1=r_2=2$: $D_w$ is trivial for $A\,A\,B\,B$ and $A\,B\,B\,A$ and is the doubled
$2$-cycle, with $\K(D_w)=\Z/2$, for $A\,B\,A\,B$ [Ch.~\ref{chap:Bio1}, Thm.~6]. For $m=3$ the four
crossing patterns give $\K(D_w)=0,\ \Z/2,\ \Z/3,\ (\Z/2)^{2}$ [Ch.~\ref{chap:Bio1}, Prop.~8]; the
...
in [Ch.~\ref{chap:Bio1}], the skeleton has $110$ and is not: the spectrum extends by one level, with
...
[Chs.~\ref{chap:Bio1}, \ref{chap:Bio2}].
