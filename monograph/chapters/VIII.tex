% generated from Paper 8/anccft8.tex
\renewcommand{\shadow}[1]{{#1}^{\sharp}}
\chapter[\texorpdfstring{VIII. The tower-level Hecke--Kirchberg limit and the non-commutative Iwasawa $K$-module}{VIII. The tower-level Hecke-Kirchberg limit and the non-commutative Iwasawa K-module}]{\texorpdfstring{Algorithmic non-commutative class field theory, VIII: the tower-level Hecke--Kirchberg limit and the non-commutative Iwasawa $K$-module}{Algorithmic non-commutative class field theory, VIII: the tower-level Hecke-Kirchberg limit and the non-commutative Iwasawa K-module}}\label{chap:VIII}
\chaptermark{VIII}
\begin{chapterabstract}
The single open problem left by [Ch.~\ref{chap:VI}] (Remark~4.3 there) asked whether the tail norms of
[Ch.~\ref{chap:V}, Thm.~3.1] lift from the sandpile groups to the shadow algebras
$\cO_{B(Y_k)}$ of the Iwahori tower, and whether the lifts assemble into one Kirchberg
algebra whose $K$-theory carries the pro-module $X_\infty$ of [Ch.~\ref{chap:V}]. We answer both
...
return arcs, not $q$: the level-$0$ normalization of [Ch.~\ref{chap:VI}] is the $(q{+}1)$-regular
...
Bowen--Franks group $\coker(I-B_k)$, where it is exactly the tail norm of [Ch.~\ref{chap:V}]; the two
...
boundary algebra of [Ch.~\ref{chap:I}] and see nothing of the tower.
\end{chapterabstract}
...
which is quoted once, in Remark~\ref{VIII:rem:down}, for an existence statement we do not
use. The in-covering lemma (Lemma~\ref{VIII:lem:incov}) and the dual-graph isomorphism
(Proposition~\ref{VIII:prop:plain}) are proved in full. Every finite identity was
machine-verified exactly on the $K_4$ tower at levels $k\le3$ (shadow matrices of size
$24$, $48$, $96$; Table~\ref{VIII:tab:battery}). Section~\ref{VIII:sec:scope} states what is
...
\section{The level shadow and its normalization}\label{VIII:sec:level}

Setting as in [Ch.~\ref{chap:V}] and [Ch.~\ref{chap:VII}]. $Y_0$ is a finite connected $(q{+}1)$-regular graph on $n$
...
\begin{equation}\label{VIII:eq:calculus}
\ta\etp=A_k,\qquad \etp\ta=A_{k+1},\qquad \ta\taup=\et\etp=qI_{n_k},\qquad
\ta\Delta_{k+1}=\Delta_k\ta,\qquad \et\Delta_{k+1}^{\,t}=\Delta_k^{\,t}\et
\end{equation}
by [Ch.~\ref{chap:V}, Prop.~1.2, Thm.~3.1, Prop.~3.2]. Transposing the last two identities,
\begin{equation}\label{VIII:eq:pullbacks}
...
\begin{convention}[Graph algebras]\label{VIII:conv}
...
\cite[Thm.~7.16]{R}; this is the convention of [Ch.~\ref{chap:VI}], where $K_0(\cO_{B(Y)})=\coker(I-B^t)$.
The other standard convention reverses all arcs. Because $T_p$ is symmetric the choice
was invisible in [Ch.~\ref{chap:VI}]; at the directed levels it is not, and we make it once and for all
...
of [Ch.~\ref{chap:V}, Prop.~3.2] (Remark~\ref{VIII:rem:mirror}).
\end{convention}

\begin{definition}[Level shadow]\label{VIII:def:shadow}
...
block products of [Ch.~\ref{chap:VI}, Thm.~2.1] give $\bigl((1+c)I-A_k^{\,t}\bigr)\oplus(-I)$; with
...
torsion groups is Lemma~\ref{VIII:lem:dual}). For the kernels: $(x,y)\in\ker(I-B_k^{\,t})$
...
quoted in [Ch.~\ref{chap:VI}, Thm.~3.1] apply verbatim. Unit class: (CK2) at $v'$ gives
$[p_{v'}]=c[p_v]+2[p_{v'}]$, i.e.\ $[p_{v'}]=-c[p_v]$, and summing over $v$ gives
$[1]=(1-c)\sum_v[p_v]=(2-q)\sum_v[p_v]$.
\end{proof}

\begin{remark}[A correction to the specification: $q-1$, not $q$]\label{VIII:rem:cq}
...
Theorem~\ref{VIII:thm:level} shows that with $c$ return arcs the Laplacian block is
...
(Table~\ref{VIII:tab:battery}, line (W)), against $\Jac(Y_1)=\Z/2\oplus\Z/8\oplus\Z/24$ and
$\Jac(Y_2)=(\Z/2)^8\oplus\Z/4\oplus\Z/16\oplus\Z/48$. The $(q{+}1)$-versus-$q$ mismatch
is the Eisenstein special layer of the whole series ([Ch.~\ref{chap:I}, Rem.~1.8], [Ch.~\ref{chap:V}, Rem.~3.3],
[Ch.~\ref{chap:VII}, Rem.~3.3]); here it fixes the number of return arcs.
\end{remark}

\begin{lemma}[Bowen--Franks duality]\label{VIII:lem:dual}
...
\section{The shadow transfer}\label{VIII:sec:transfer}

\begin{lemma}[In-coverings induce homomorphisms]\label{VIII:lem:incov}
...
\begin{proposition}[The tail map is an in-covering of shadows]\label{VIII:prop:tausharp}
...
\begin{theorem}[Shadow transfer]\label{VIII:thm:transfer}
...
isomorphisms of Theorem~\ref{VIII:thm:level} is the pullback
...
(Lemma~\ref{VIII:lem:dual}), hence injective; on the torsion-free quotient
...
\begin{proof}
Lemma~\ref{VIII:lem:incov} applies by Proposition~\ref{VIII:prop:tausharp}. (i) The intertwining
reduces blockwise to $\taup A_k^{\,t}=A_{k+1}^{\,t}\taup$, the transpose of
$\ta A_{k+1}=A_k\ta$ \eqref{VIII:eq:calculus}; $\taup\mathbf1=\mathbf1$ because every vertex
...
Theorem~\ref{VIII:thm:level} carry the map induced by $\Tup$ to the map induced by $\taup$ on
$\coker\Delta^{t}$. The dual of a surjection is injective, and $\ta$ is surjective on
$\Jac$ [Ch.~\ref{chap:V}, Thm.~3.1(ii)]. Degree: $\deg\circ\Delta^{t}=(\Delta\mathbf1)^t=0$ so $\deg$
...
Lemma~\ref{VIII:lem:incov} with (i).
\end{proof}

\begin{theorem}[The transfer matrix and the tail norm]\label{VIII:thm:matrix}
...
under $V^{t}$ of \eqref{VIII:eq:factor} this map is exactly the tail norm
$\ta\colon\coker\Delta_{k+1}\to\coker\Delta_k$ of {\rm[Ch.~\ref{chap:V}, Thm.~3.1]}: surjective,
...
multiplication by $q=\ind(\ta)$, the Eisenstein index of {\rm[Ch.~\ref{chap:VII}, Cor.~2.2]};
...
the properties listed are [Ch.~\ref{chap:V}, Thm.~3.1(ii)]. (ii) $\ta\mathbf1=q\mathbf1$. (iii) is
Lemma~\ref{VIII:lem:dual}(ii) with $F=\ta$. (iv) By \eqref{VIII:eq:calculus},
...
\begin{remark}[The mirror]\label{VIII:rem:mirror}
Under Convention~\ref{VIII:conv} exactly one degeneracy map lifts to the $C^*$-level: the one
...
($\et A_{k+1}^{\,t}=A_k^{\,t}\et$ by \eqref{VIII:eq:calculus}), not $\ta$
(Theorem~\ref{VIII:thm:matrix}(iv)). Reversing all arcs exchanges $\tau\leftrightarrow\eta$,
$\Delta\leftrightarrow\Delta^{t}$, and the pro-module $X_\infty$ of [Ch.~\ref{chap:V}, Thm.~3.1] with
the in-module $X_\infty^{\mathrm{in}}$ of [Ch.~\ref{chap:V}, Prop.~3.2]. This is the $C^*$-algebraic
face of the two-sidedness proved in [Ch.~\ref{chap:V}]: the tower is a $(\tau,\eta)$-bimodule system,
and each side is realized by one of the two conventions.
\end{remark}

\begin{remark}[Downward homomorphisms]\label{VIII:rem:down}
The specification asked for $*$-homomorphisms $\cO_{k+1}\to\cO_k$ inducing $\ta$. Under
Convention~\ref{VIII:conv} no map $\cO_{k+1}\to\cO_k$ can induce $\ta$ on $K_0$, because $\ta$
does not descend (Theorem~\ref{VIII:thm:matrix}(iv)); the descending pushforward is $\et$,
...
\begin{proposition}[Prime-to-$q$ stability]\label{VIII:prop:stable}
...
\begin{proof}
$\etp$ descends by \eqref{VIII:eq:pullbacks}; on $\coker\Delta_k$ one has $A_k\equiv qI$
since $\Delta_k=qI-A_k\equiv0$; the two composites are \eqref{VIII:eq:calculus}. On the
$K_4$ tower the odd part is $\Z/3$ at $k=1,2,3$ (Table~\ref{VIII:tab:battery}).
\end{proof}

\begin{theorem}[The kernel of the tail norm is the $p$-torsion]\label{VIII:thm:ptorsion}
...
the module $X_\infty$ of {\rm[Ch.~\ref{chap:V}, Thm.~3.1]}. Then $X_\infty^{(p)}$ is a torsion-free
...
in $\Jac(Y_{k+1})[p]$ by Proposition~\ref{VIII:prop:stable}; its order is $p^{\,n_k(p-1)-1}$ by
[Ch.~\ref{chap:V}, Thm.~3.1(ii)] (the increment of [Ch.~\ref{chap:III}, Thm.~2.1], re-proved homologically in
[Ch.~\ref{chap:VII}, Cor.~2.2]); and $|\Jac(Y_{k+1})[p]|=p^{\,n_k(p-1)-1}$ by (i). (iii) Let
...
Proposition~\ref{VIII:prop:stable}. Machine confirmation on $K_4$ ($p=2$):
...
orders (Table~\ref{VIII:tab:battery}, line (Q)).
\end{proof}
...
Proposition~\ref{VIII:prop:stable} turned the coincidence into a theorem. Its meaning for the
Iwasawa-theoretic reading of Chs.~\ref{chap:III}--\ref{chap:V} is discussed in Remark~\ref{VIII:rem:euler}. Note
that (ii) has no meaning at the boundary transition $0\to1$: there $\ta$ does not
descend to the cokernels ([Ch.~\ref{chap:V}, Rem.~3.3]) and $A_1\ne\etp\ta$ ([Ch.~\ref{chap:I}, Def.~1.7]:
...
\section{The limit algebra}\label{VIII:sec:limit}

\begin{definition}\label{VIII:def:limit}
$\cO_\infty(Y_\bullet):=\varinjlim_k\bigl(\cO_k,\varphi_k\bigr)$, the $C^*$-inductive limit
of the level shadow algebras along the shadow transfers of Theorem~\ref{VIII:thm:transfer},
starting at $k=1$. We abbreviate it $\cO_\infty$.
\end{definition}

\begin{theorem}[The limit is Kirchberg]\label{VIII:thm:kirchberg}
...
$\varphi_k$ are gauge-equivariant (Lemma~\ref{VIII:lem:incov}).
\end{proof}

\begin{theorem}[$K$-theory of the limit]\label{VIII:thm:klimit}
...
for $p\mid q$ is the torsion of $X_\infty$ of {\rm[Ch.~\ref{chap:V}, Thm.~3.1]}). Then:
\begin{enumerate}
...
along $K_*(\varphi_k)$, computed in Theorem~\ref{VIII:thm:transfer}. (i) The maps are the
identity. (ii) Injectivity is Theorem~\ref{VIII:thm:transfer}(ii); the torsion of $K_0(\cO_k)$
is $\Tor_k^{\,t}\cong\Jac(Y_k)^{\vee}$ by Lemma~\ref{VIII:lem:dual}. (iii) Direct limits are
exact, so $\Tor\varinjlim=\varinjlim\Tor$ and $\varinjlim(G_k)/\Tor=\varinjlim(G_k/\Tor)
=\varinjlim(\Z,\times q)=\Z[1/q]$ (Theorem~\ref{VIII:thm:transfer}(ii)). Pontryagin duality
exchanges inverse limits of compact groups with direct limits of their discrete duals,
and $\taup$ is dual to $\ta$ by Lemma~\ref{VIII:lem:dual}(ii). The recovery statement is
double duality for the discrete torsion group $\XX^{\vee}$. (iv) By
Theorem~\ref{VIII:thm:ptorsion}(iii), $\XX^{\vee}=D\oplus F$ with $D$ divisible (hence an
...
splits. (v) Theorem~\ref{VIII:thm:level} and $\taup\sum_ve_v=\sum_we_w$; $\deg u=n_k\ne0$ at
level $k$.
\end{proof}

\begin{corollary}[The pro-module in $K$-homology]\label{VIII:cor:khom}
...
\begin{proof}
Apply $\Hom(-,\Z)$ to the sequence of Theorem~\ref{VIII:thm:klimit}(iii): $\Hom(\XX^{\vee},\Z)=0$
...
\begin{theorem}[No-Go: the pro-module is not a $K$-group]\label{VIII:thm:nogo}
...
dual one, Theorem~\ref{VIII:thm:klimit}(iii).
\end{theorem}

\begin{proof}
$\XX\twoheadrightarrow\Jac(Y_k)$ for all $k$ with $|\Jac(Y_k)|\to\infty$ (the
pseudo-Iwasawa law [Ch.~\ref{chap:III}, Thm.~2.2]), so $\XX$ is an infinite profinite group. A
...
\section{Spectral blindness, broken at the tower level}\label{VIII:sec:blind}

\begin{proposition}[The plain tower algebras are constant]\label{VIII:prop:plain}
...
\]
the boundary-channel groups of {\rm[Ch.~\ref{chap:I}, Thm.~3.7]}; the rooted tail groupoid algebra of
{\rm[Ch.~\ref{chap:I}, Prop.~1.9(iii)]} and {\rm[Ch.~\ref{chap:III}, \S1]} has $K_0\cong\Z/(q-1)$, $K_1=0$. Neither sees
$\Jac(Y_k)$ for any $k$.
\end{proposition}

\begin{proof}
In-arcs at $f\in L(Y_k)^0$ are the $(e,f)$ with $r(e)=s(f)$, mapping bijectively to the
in-arcs $e$ of $\tau(f)=s(f)$: Lemma~\ref{VIII:lem:incov} gives $\tau^\natural$. The family
...
$C(\partial X)\rtimes\Gamma$ [Ch.~\ref{chap:I}, Prop.~1.9(ii)]. On $K_4$: $\coker(I-A_k^{\,t})=\Z^3\oplus\Z/2$
at $k=1,2,3$, $r=3$ (Table~\ref{VIII:tab:battery}, line (B)).
\end{proof}
...
\midrule
rooted tail groupoid $C^*_r(\mathcal G_\infty)$ [Chs.~\ref{chap:I}, \ref{chap:III}] & $\Z/(q-1)$ & $0$ & none\\
...
$K_0(\varphi_k)$.}
\label{VIII:tab:compare}
\end{table}

\begin{remark}[Where $\mu$, $-k$ and the Euler line live]\label{VIII:rem:euler}
The pseudo-Iwasawa law $\ord_p|\Jac(Y_k)|=\mu p^k-k+\nu_0$, $\mu=n(q{+}1)/q$
[Ch.~\ref{chap:III}, Thm.~2.2], is the growth law of the torsion of the level filtration of
$K_0(\cO_\infty)$ (Theorem~\ref{VIII:thm:klimit}(ii)): $7,18,41$ on $K_4$. The Eisenstein index
$\ind(\ta)=q$ of [Ch.~\ref{chap:VII}] appears three times in the limit: as the action of $\Tdn$ on
$\ker(I-B)$ (Theorem~\ref{VIII:thm:matrix}(ii)), as the scaling of the free part of $K_0$ that
produces $\Z[1/q]$ (Theorem~\ref{VIII:thm:transfer}(ii)), and as the $q$-adic term
$\Ext(\Z[1/q],\Z)$ separating $K$-homology from the pro-module (Corollary~\ref{VIII:cor:khom}).
Theorem~\ref{VIII:thm:ptorsion} sharpens the arithmetic reading: for $q=p$ the $p$-rank of
$\Jac(Y_{k+1})$ is $(n_{k+1}-n_k)-1$, the arc increment minus the Eisenstein line, exactly
the two terms of the snake formula [Ch.~\ref{chap:VII}, eq.~(2.1)] now read as a rank count rather than
...
[Ch.~\ref{chap:III}, Prop.~2.3] ($\lambda=-1$) and [Ch.~\ref{chap:V}, Prop.~4.1] ($A_kA_k^{\,t}\ne A_k^{\,t}A_k$), that
the classical mechanism is not the one at work. We state it as a certificate, not as a
theory: Problem~5.1 of [Ch.~\ref{chap:V}] and its residue [Ch.~\ref{chap:VII}, Rem.~3.1] are untouched by this paper.
\end{remark}

\section{Scope, residue, corrections}\label{VIII:sec:scope}

\begin{remark}[Solved, open, impossible]\label{VIII:rem:scope}
\emph{Solved:} the tower-level functoriality of [Ch.~\ref{chap:VI}, Rem.~4.3]. The tail norms lift
...
$\shadow\eta$ with $K_0$-map $\et$; existence is abstract (Remark~\ref{VIII:rem:down}),
canonicity is not established; (b) whether the sequence of
Theorem~\ref{VIII:thm:klimit}(iii) splits for composite $q$; (c) the Hecke-module structure of
$\XX$ as a module over the degeneracy pro-ring of [Ch.~\ref{chap:V}, Prop.~4.1], and with it
[Ch.~\ref{chap:V}, Prob.~5.1]. \emph{Impossible:} a separable $C^*$-algebra with $K_0\supseteq X_\infty$
(Theorem~\ref{VIII:thm:nogo}); and, as before, a Hecke-dynamical realization
([Ch.~\ref{chap:II}, Thm.~1.6], [Ch.~\ref{chap:VI}, Rem.~4.3]).
\end{remark}

\begin{remark}[Four corrections to the task specification]\label{VIII:rem:corrections}
(1) The level shadow needs $q-1$ return arcs, not $q$ (Remark~\ref{VIII:rem:cq}). (2) The
matrix $\Tdn=\operatorname{diag}(\ta,\ta)$ does not induce a map
$\coker(I-B_{k+1}^{\,t})\to\coker(I-B_k^{\,t})$ (Theorem~\ref{VIII:thm:matrix}(iv)); it induces
...
$K_0(\cO_\infty)\cong\Z\oplus X_\infty$ is impossible (Theorem~\ref{VIII:thm:nogo}); the
...
[Ch.~\ref{chap:I}, Prop.~1.9(iii)] and [Ch.~\ref{chap:III}, \S1]; [Ch.~\ref{chap:I}, Thm.~3.7] is the boundary channel, with
$K_0\cong\Z^r\oplus\Z/(r-1)$; both are blind, and the plain tower algebras are the latter
at every level (Proposition~\ref{VIII:prop:plain}). We also decline the specification's
historical superlative; what is proved is stated in Remark~\ref{VIII:rem:scope}.
\end{remark}

\begin{remark}[Boundary level]\label{VIII:rem:boundary}
...
$c=q-1$ at level $1$), the intertwining fails by the degeneracy boundary [Ch.~\ref{chap:V}, Rem.~3.3],
and $A_1\ne\etp\ta$. As everywhere in the series the base enters as the initial value
$\nu_0$, and the limit $\cO_\infty$ starts at $k=1$. The single-level algebra of [Ch.~\ref{chap:VI}] at
level $0$ stands beside the tower, not at its foot.
\end{remark}

\section{The verification battery}\label{VIII:sec:battery}

Table~\ref{VIII:tab:battery} lists the exact checks performed by \texttt{tower\_kirchberg.py};
the letters match the docstring of the script and the references in the text. The
levels $k\le3$ of the $K_4$ tower are the same ones used in [Ch.~\ref{chap:V}] and [Ch.~\ref{chap:VII}], so every
...
(Remark~\ref{VIII:rem:down}). Theorem~\ref{VIII:thm:ptorsion} was found by machine and then proved;
it is stated for $q=p$ prime, and for composite $q$ only the inclusion
$\ker\ta\subseteq\Jac[q]$ is claimed. Remark~\ref{VIII:rem:corrections} records the
