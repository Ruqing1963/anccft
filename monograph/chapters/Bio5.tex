% generated from Bio 5/ecoli_decomposition.tex
\chapter{The bundle-chain decomposition of a bacterial genome: \emph{Escherichia coli} K-12 at five $k$-mer lengths}\label{chap:Bio5}
\chaptermark{Bio5}
...
\section{Introduction}\label{Bio5:sec:intro}
...
group [Ch.~\ref{chap:Bio1}]. The bundle-chain splitting theorem [Ch.~\ref{chap:Bio3}] writes
\[
...
\section{Why a 4.6\,Mbp genome is tractable}\label{Bio5:sec:scaling}

\begin{lemma}[Branch vertices are the repeated $k$-mers]\label{Bio5:lem:branch}
...
--- i.e.\ is smoothed away by the unitig contraction of [Ch.~\ref{chap:Bio1}, Lem.~4] --- if and
...
$34\,272$ (Table~\ref{Bio5:tab:main}), and the decomposition of [Ch.~\ref{chap:Bio3}] then runs in one
pass, since the chain-step relation is a partial injection [Ch.~\ref{chap:Bio3}, Lem.~6].
...
\section{Results}\label{Bio5:sec:results}
...
contraction. ``branch'' is the number of repeated $k$-mers (Lemma~\ref{Bio5:lem:branch}) and
...
\label{Bio5:tab:groups}
\end{table}

\begin{remark}[The shape of the answer]\label{Bio5:rem:shape}
Three things are worth reading off Table~\ref{Bio5:tab:main}.
...
\section{The ribosomal operons, and a correction}\label{Bio5:sec:rrna}
...
[Chs.~\ref{chap:Bio1}, \ref{chap:Bio2}, \ref{chap:Bio3}, \ref{chap:Bio4}] is single-strand: two copies are a repeat of $G_k(S)$ only if
...
\begin{remark}[What this measures]\label{Bio5:rem:bidirected}
A bidirected theory --- the open problem of [Ch.~\ref{chap:Bio2}, Open (a)] --- would see all seven
...
\section{Verification}\label{Bio5:sec:battery}

The script \texttt{ecoli\_sandpile.py} runs the six checks of Table~\ref{Bio5:tab:battery} in
...
\label{Bio5:tab:battery}
\end{table}

Check~(V) deserves a word. The pipeline used here is not the one used in [Ch.~\ref{chap:Bio3}]: it
...
\section{Scope}\label{Bio5:sec:scope}
...
\emph{Single-strand.} As in [Chs.~\ref{chap:Bio1}, \ref{chap:Bio2}, \ref{chap:Bio3}, \ref{chap:Bio4}]. Section~\ref{Bio5:sec:rrna} is the
...
[Ch.~\ref{chap:Bio3}] that is what the sandpile group of a generic digraph looks like, and no
...
Unrefereed preprint. Lemma~\ref{Bio5:lem:branch} is elementary and is stated because it is
...
The expectation of a multiplicity-seven rRNA family, which Section~\ref{Bio5:sec:rrna}
