% generated from Paper 20/anccft20.tex
\renewcommand{\LE}{L_E}
\chapter[\texorpdfstring{XX. The invariant subspace in rank $d$, and one flag count that produces it}{XX. The invariant subspace in rank d, and one flag count that produces it}]{\texorpdfstring{Algorithmic non-commutative class field theory, XX: the invariant subspace in rank $d$, and one flag count that produces it}{Algorithmic non-commutative class field theory, XX: the invariant subspace in rank d, and one flag count that produces it}}\label{chap:XX}
\chaptermark{XX}
...
Paper XVIII proved
\begin{equation}\label{XX:eq:main}
...
\begin{definition}\label{XX:def:M}
...
\begin{equation}\label{XX:eq:inside}
...
\begin{theorem}\label{XX:thm:flag}
...
Theorem~\ref{XX:thm:flag} is verified exhaustively --- over \emph{every} pair $(V,H)$ --- for
$(d,q)$ equal to $(2,2)$, $(2,3)$, $(3,2)$, $(3,3)$ and $(4,2)$ and every $i$, fourteen
triples in all; checks
(F) and (K). Table~\ref{XX:tab:flag} lists the values.
...
\begin{lemma}[Paper XVIII, Lem.~2.2]\label{XX:lem:link}
...
\begin{theorem}\label{XX:thm:inter}
...
$0\le i\le d$,
\begin{equation}\label{XX:eq:rec}
  \LE N_i^{\tp}=q^{\,i}\bigl(N_0^{\tp}A_{i+1}-N_{i+1}^{\tp}\bigr),
\end{equation}
...
$f(i+1)=[i+1]_qf(i)$,
\begin{equation}\label{XX:eq:kappa}
  \frac{K_i}{c_i}
...
\]
the condition $z\notin x$ being Lemma~\ref{XX:lem:link}. Work in the link of $y$. There $x$ is a
...
\[
  ...
  =f(i+1)f(d-i)-N(V,H).
\]
By Theorem~\ref{XX:thm:flag} this is $0$ when $V\subseteq H$ and $K_i$ otherwise. Now
...
$A_{i+1}(y,v)$. Assembling and dividing by $c_i$, which is $q^{\,i}$ by \eqref{XX:eq:kappa},
\[
...
and reverses inclusion, carries this count to the count of Theorem~\ref{XX:thm:flag} with $i$
...
Finally, each right-hand side of \eqref{XX:eq:rec} is a $\Z$-linear combination of the blocks
...
\begin{corollary}\label{XX:cor:W}
...
\begin{proof}
The inclusion is \eqref{XX:eq:inside}. If $\LE\Psi_d=\Psi_dC$ then $\LE(\im\Psi_d)\subseteq
\im\Psi_d$, so $\LE^{\tp}$ preserves $(\im\Psi_d)^{\perp}$; and $\LE^{\tp}\Psi_d=\Psi_dC'$
gives the other half. Now apply \eqref{XX:eq:main}, whose correction terms $T^{\tp}T$ and
$S^{\tp}S$ vanish on $W_d$.
\end{proof}

\section{\texorpdfstring{The companion matrix, and the Hecke polynomial in rank $d$}{The companion matrix, and the Hecke polynomial in rank d}}

Reading \eqref{XX:eq:rec} columnwise gives $C$ outright: the block in row $0$, column $i$ is
$q^{\,i}A_{i+1}$ and the block in row $i+1$, column $i$ is $-q^{\,i}I$, all others zero.

\begin{theorem}\label{XX:thm:hecke}
...
\begin{proof}
The shape of $C$ is \eqref{XX:eq:rec}. For the determinant, eliminate: \eqref{XX:eq:rec} gives
...
symbolically for $d=2,3,4,5$, and \eqref{XX:eq:kappa} for $d\le6$; checks (H) and (A).

\begin{theorem}\label{XX:thm:corank}
...
On all three complexes of Section~\ref{XX:sec:y} the corank is exactly $d(d+1)=6$ and these
...
\section{Rank two, and the Hecke pencil}\label{XX:sec:y}

At $d=2$ we have $N_0=T$, $N_2=S$ and $N_1=M$, the chamber indicator; the three cases of
\eqref{XX:eq:rec} read
\begin{equation}\label{XX:eq:three}
...
built on it is Kang and Li's \cite{KL14}. The three identities of \eqref{XX:eq:three}, their
...
checks (I) and (P). Table~\ref{XX:tab:y} records the subspace they produce.
...
\begin{remark}[What the decomposition says]\label{XX:rem:split}
Corollary~\ref{XX:cor:W} splits the spectrum of $\LE$ into a part carried by $\im\Psi_d$ and a
...
$q^{-1/2}$ family is precisely the $W_d$ part, and Corollary~\ref{XX:cor:W} says it is there for
every complex, Ramanujan or not; the Ramanujan content is entirely in the $q^{-1}$ family,
which is the Hecke part.
\end{remark}

\begin{remark}[Solved, open, impossible]\label{XX:rem:soi}
...
Theorem~\ref{XX:thm:corank} is an equality for $d\ge3$, hence $\dim W_d$ exactly; at $d=2$ it is
...
Hecke part is where Ramanujan lives, and Corollary~\ref{XX:cor:W} is silent about it by
...
(I) & the six rank-2 identities of \eqref{XX:eq:three} and their mirrors & 3 complexes & exact\\
...
\paragraph{Scope.} Theorem~\ref{XX:thm:flag} and Theorem~\ref{XX:thm:inter} are proved for every
$d\ge2$ and every $\Atilde d$ complex of order $q$; so is Corollary~\ref{XX:cor:W}. The global
identities are \emph{verified} only at $d=2$, because we have no $\Atilde d$ complex for
$d\ge3$ to verify them on; the proof of Theorem~\ref{XX:thm:inter}, however, uses nothing beyond
Lemma~\ref{XX:lem:link} and the flag count, and both hold in every rank. The corank of $\Psi_d$
is not determined.

\paragraph{Provenance.} Identity~\eqref{XX:eq:main} and Lemma~\ref{XX:lem:link} are Paper XVIII.
The $d=2$ determinant identity
$Z(X,u)^{-1}=\det(I-A_1u+qA_2u^{2}-q^{3}u^{3})\cdot\det(I+L_Bu)\cdot(1-u^{3})^{-\chi}$
and the four-way Ramanujan criterion quoted in Remark~\ref{XX:rem:split} are Kang and Li
