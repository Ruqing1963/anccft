% generated from Bio 4/multicopy.tex
\chapter{Multi-copy repeats: where the two-copy theory of assembly ambiguity stops}\label{chap:Bio4}
\chaptermark{Bio4}
...
[Ch.~\ref{chap:Bio3}]. For repeats of multiplicity two, $\K(D_w)$ is the critical group of a
...
\section{Introduction}\label{Bio4:sec:intro}

Let $S$ be a circular sequence with repeat families $W_1,\dots,W_m$ of multiplicities
$r_i$, clean in the sense of [Ch.~\ref{chap:Bio1}], and let $w$ be the circular word of their
occurrences. The bundle-chain splitting theorem [Ch.~\ref{chap:Bio3}] gives
\[
...
[Ch.~\ref{chap:Bio3}] is $2$-in $2$-out at $k=11$ and $k=10$ but has vertices of out-degree $3$ and
$4$ at $k=9$ and up to $6$ at $k=7$ (Table~\ref{Bio4:tab:phix}). This note asks whether the
...
digraph, whatever the degrees, so the torsor of [Ch.~\ref{chap:Bio2}] generalises even though, as
we show, the matrix presentation does not.

\section{Setting}\label{Bio4:sec:setting}
...
\section{Two families}\label{Bio4:sec:two}

\begin{theorem}[Two families]\label{Bio4:thm:two}
...
For $r=s=2$ this is the interleaving theorem [Ch.~\ref{chap:Bio1}, Thm.~6]: $c=1$ for
...
\section{Beyond two families nothing pairwise survives}\label{Bio4:sec:neg}

\begin{proposition}[Pairwise patterns do not determine the group]\label{Bio4:prop:pair}
The circular words
\[
...
\begin{remark}[Why this is sharper than the two-copy failure]\label{Bio4:rem:sharper}
...
classes of Proposition~\ref{Bio4:prop:pair}, which fail already at $m=3$. There is no room
between the two for a sign.
\end{remark}

\begin{proposition}[No reduction to two copies]\label{Bio4:prop:detach}
...
transition digraph, which is what [Ch.~\ref{chap:Bio3}] says. The content of the two-copy theory is
that $T$ can be traded for interlacement --- a different, topological parametrisation of
the same group. Propositions~\ref{Bio4:prop:pair} and~\ref{Bio4:prop:detach} say that for three or
...
\section{Census}\label{Bio4:sec:census}

Table~\ref{Bio4:tab:census} lists, for each multiplicity vector, the number of circular words
...
$c\le\min(r,s)$, as Theorem~\ref{Bio4:thm:two} predicts, and every pairwise class is pure.
From three families on the arrangement group is far richer than for two-copy repeats
--- three two-copy repeats give only $0,\Z/2,\Z/3,(\Z/2)^{2}$ [Ch.~\ref{chap:Bio1}, Prop.~8],
...
\label{Bio4:tab:census}
\end{table}

\section{$\varphi$X174}\label{Bio4:sec:phix}

Table~\ref{Bio4:tab:phix} gives the out-degree profile of the bundle-contracted skeleton of
...
$\Z/4\oplus\Z/1076338579$, the groups computed in [Ch.~\ref{chap:Bio3}] from the Laplacian
...
\section{Verification}\label{Bio4:sec:battery}

The script \texttt{multicopy.py} runs the four checks of Table~\ref{Bio4:tab:battery} in
...
key & check & mode\\
\midrule
(A) & Theorem~\ref{Bio4:thm:two} on all $119$ two-letter circular words, $r,s\le5$ & exact\\
(B) & Table~\ref{Bio4:tab:census}; the witness of Proposition~\ref{Bio4:prop:pair} & exact\\
(C) & Proposition~\ref{Bio4:prop:detach}: $150$ digraphs, $1350$ detachments; the witness & exact\\
(D) & Table~\ref{Bio4:tab:phix}; $\coker(A+I)=\K(\Sk)$ at $k=11,10$ & exact\\
\bottomrule
\end{tabular}
\caption{The verification battery, $4/4$.}
\label{Bio4:tab:battery}
\end{table}

\section{Scope}\label{Bio4:sec:scope}

\emph{Proved.} Theorem~\ref{Bio4:thm:two}; Propositions~\ref{Bio4:prop:pair}
and~\ref{Bio4:prop:detach} by explicit witness.

\emph{Quoted.} The splitting theorem [Ch.~\ref{chap:Bio3}]; the two-copy presentation \cite{MMN};
...
counts arborescences, has a cokernel equal to $\K$. Theorem~\ref{Bio4:thm:two} shows that at
least for two families the group is a crossing number, so a multi-chord crossing calculus
is not ruled out; Proposition~\ref{Bio4:prop:pair} shows it cannot be pairwise. (b) The
...
form used here; a modular Smith form would extend Table~\ref{Bio4:tab:phix} of [Ch.~\ref{chap:Bio3}].

\section*{Provenance of this chapter}

Unrefereed preprint. Theorem~\ref{Bio4:thm:two} is elementary and, as far as we can find,
...
multi-copy repeats was the obvious next step after [Ch.~\ref{chap:Bio3}], and it does not exist in
