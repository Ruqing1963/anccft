% generated from Paper 12/anccft12.tex
\chapter[\texorpdfstring{XII. $\Atwo$ towers --- no abelian sector, the covering norm theorem, and the two digraph towers}{XII. A2 towers -- no abelian sector, the covering norm theorem, and the two digraph towers}]{\texorpdfstring{Algorithmic non-commutative class field theory, XII: $\Atwo$ towers --- no abelian sector, the covering norm theorem, and the two digraph towers}{Algorithmic non-commutative class field theory, XII: A2 towers -- no abelian sector, the covering norm theorem, and the two digraph towers}}\label{chap:XII}
\chaptermark{XII}
...
$\Z_\ell$-voltage towers in rank two, the commutative Iwasawa theory of [Ch.~\ref{chap:IV}] has no rank-two
...
line-digraph recursion of [Ch.~\ref{chap:III}] with degree $q^{2}$ and $q$, giving the exact laws
...
parahoric factor, by the edge-flow theorem of [Ch.~\ref{chap:XI}]. A two-dimensional degeneracy calculus for
...
exactly), the line-digraph recursion [Ch.~\ref{chap:III}, Thm.~2.1], the edge-flow theorem [Ch.~\ref{chap:XI}, Thm.~3.3],
and the norm-surjectivity argument of [Ch.~\ref{chap:IV}, Lem.~1.3]. All Smith forms are $p$-adic with the
prime sets certified as in [Ch.~\ref{chap:X}, \S6] or by determinants of maximal minors
(Section~\ref{XII:sec:noabelian}). Section~\ref{XII:sec:scope} records what is open and the
corrections to the specification.

\section{No abelian sector in rank two}\label{XII:sec:noabelian}

\begin{theorem}[No abelian towers]\label{XII:thm:noabelian}
...
finite group $H_1(Y;\Z)$, and the limit modules $X_\infty$ of {\rm[Ch.~\ref{chap:IV}]} have no rank-two
analogue.
\end{theorem}
...
$256$); on $Y_{84},Y_{168}$, $b_1=0$ by exact ranks (Table~\ref{XII:tab:battery}, line (B)).
\end{proof}

\begin{remark}[What this changes]\label{XII:rem:framework}
...
Iwahori tower of [Ch.~\ref{chap:III}], [Ch.~\ref{chap:V}], [Ch.~\ref{chap:VII}], [Ch.~\ref{chap:VIII}] was the non-normal exception. In rank two the
...
theory'' is therefore, from the start, the non-commutative theory that [Ch.~\ref{chap:V}] and [Ch.~\ref{chap:VII}] asked
for in rank one; nothing commutative can be borrowed.
\end{remark}

\section{The finite abelian chains and the covering norm theorem}\label{XII:sec:chain}

The complexes of [Ch.~\ref{chap:X}] are abelian covers of the three-vertex type quotient $X_3$ of the
...
\begin{theorem}[Covering norm theorem]\label{XII:thm:norm}
...
argument of [Ch.~\ref{chap:IV}, Lem.~1.3] applies verbatim: $P_\pi$ is surjective, $\mathbf1^{t}\DeltaH=0$
makes $\deg$ well defined, $\deg\circ\pi_*=\deg$, and $\Tor\coker\DeltaH=\ker\deg$ by
[Ch.~\ref{chap:X}, Thm.~3.2(ii)]; surjectivity on torsion follows, and the kernel order is forced by
...
$Y_k$. Apply [Ch.~\ref{chap:X}, Thm.~3.2(ii)] at both levels and divide. Verified numerically to $10^{-6}$
on the five coverings, together with the exact integrality of the ratios and the exact
intertwining identities (Table~\ref{XII:tab:battery}, line (A)).
\end{proof}
...
\oplus\Z/29360114\oplus\Z/117440456\oplus\Z/352321368$ (new here); the others are in [Ch.~\ref{chap:X}].
...
\section{Two-dimensional degeneracy calculus}\label{XII:sec:calculus}

Let $S,T,M\in M_{n\times|E|}(\Z)$ be the tail, head and third-vertex incidence matrices of
[Ch.~\ref{chap:XI}, \S3], and $d_0,d_1,d_2\in M_{|E|\times|F|}(\Z)$ the three face maps sending a chamber
$(a,b,c)$ (types $0,1,2$) to its typed edges $(a\to b)$, $(b\to c)$, $(c\to a)$.

\begin{theorem}[Incidence calculus in rank two]\label{XII:thm:calculus}
For an $\Atwo$ complex satisfying {\rm[Ch.~\ref{chap:X}, Def.~1.1(i)--(iv)]} with $\PG(2,q)$ links:
\begin{align*}
...
\begin{proof}
The first line is [Ch.~\ref{chap:XI}, proof of Thm.~3.3] and Lemma~[Ch.~\ref{chap:XI}, 3.2(a)]: $(SM^{t})(x,w)$ counts
...
in [Ch.~\ref{chap:XI}, Def.~3.1]. Verified exactly on $Y_{168}$ (Table~\ref{XII:tab:battery}, line (D)).
\end{proof}

\begin{remark}[The three degeneracy operators]\label{XII:rem:degeneracy}
In rank one, [Ch.~\ref{chap:V}] had two degeneracy maps $\tau,\eta$ with $\ta\etp=A_k$. In rank two the
...
\section{The two digraph towers}\label{XII:sec:towers}

\begin{definition}[Edge and chamber towers]\label{XII:def:towers}
For an $\Atwo$ complex $Y$ with the hypotheses of Theorem~\ref{XII:thm:calculus} let
$E_1:=(E,\LE)$ be the geodesic edge digraph and $F_1:=(F^{\pm},\LF)$ the chamber digraph of
[Ch.~\ref{chap:XI}, Rem.~3.6] (oriented chambers $(a,b,c)\to(b,c,d)$, $d\ne a$), and for $k\ge1$ let
...
\begin{theorem}[Recursions and growth laws]\label{XII:thm:towers}
$E_1$ is Eulerian $q^{2}$-regular and strongly connected, $F_1$ is Eulerian $q$-regular and
strongly connected (exact on $Y_{168}$; in general by {\rm[Ch.~\ref{chap:XI}, Lem.~3.2(a)]} and its chamber
analogue). Hence, by {\rm[Ch.~\ref{chap:III}, Thm.~2.1]} applied with $d=q^{2}$ and $d=q$, for all $k\ge1$
\[
...
spectrum {\rm[Ch.~\ref{chap:XI}, Thm.~3.3]}.
\end{theorem}

\begin{proof}
Regularity: each typed edge has $q^{2}$ continuations and $q^{2}$ predecessors
([Ch.~\ref{chap:XI}, Lem.~3.2(a)] and its reverse); each oriented chamber has $q$ successors and $q$
...
vertices, with $\det(zI-A_{L(G)})=z^{m-n}\det(zI-A_G)$ [Ch.~\ref{chap:III}, eq.~(2.1)]; the recursion is
[Ch.~\ref{chap:III}, Thm.~2.1]; summing, $\sum_{j<k}2(n_j(E)(q^{2}-1)-1)=2n_1(E)(q^{2(k-1)}-1)-2(k-1)$ gives
the edge law and likewise the chamber law. Sanity check of [Ch.~\ref{chap:III}, Thm.~2.1] with $d=4$ on the
complete digraph $K_5$: $\kappa=125$, $\kappa(L)=4^{14}\cdot125$ (Table~\ref{XII:tab:battery},
line (L)).
\end{proof}

\begin{remark}[The higher Euler term]\label{XII:rem:chi2}
The specification predicted $\chi_2=2$. It is right, and for a transparent reason: in
[Ch.~\ref{chap:IX}, Thm.~4.1(iv)] the Euler term of the rank-one Iwahori tower was identified as
...
\begin{proposition}[The sandpile of the geodesic edge digraph]\label{XII:prop:kappaE1}
With the notation of {\rm[Ch.~\ref{chap:XI}, Cor.~3.5]},
\[
...
\begin{proof}
$E_1$ is Eulerian and strongly connected, so [Ch.~\ref{chap:III}, eq.~(2.2)] gives
$n_1\kappa(E_1)=\prod_{\mu\ne0}\mu$ over the nonzero eigenvalues of $q^{2}I-\LE$, i.e.\ over
all eigenvalues $\lambda\ne q^{2}$ of $\LE$. By [Ch.~\ref{chap:XI}, Cor.~3.5] these are: the two rotations
...
\section{The verification battery}\label{XII:sec:battery}
...
(D) the fourteen identities of Theorem~\ref{XII:thm:calculus}, including $A_1A_2=A_2A_1$ & $Y_{168}$ & exact\\
...
(L) [Ch.~\ref{chap:III}, Thm.~2.1] with $d=4$ on $K_5$: $\kappa(L)=4^{14}\kappa$ & --- & exact\\
\bottomrule
\end{tabular}}
\medskip
\caption{Verification (\texttt{pgl3\_tower.py}).}
\label{XII:tab:battery}
\end{table}

\section{Scope, residue, corrections}\label{XII:sec:scope}

\begin{remark}[Solved, open, impossible]\label{XII:rem:scope}
...
$168\cdot|H_1(Y_{168})|$ vertices), and Theorem~\ref{XII:thm:noabelian} shows no abelian
shortcut exists; (b) an exact value of $\kappa(E_1)$ on $Y_{168}$ and the $p$-adic constant
$\nu_E$; (c) a closed form for the parahoric factor of Proposition~\ref{XII:prop:kappaE1}, which
should be Kang--Li's chamber factor at $u=q^{-2}$; (d) the pro-module of an infinite
congruence tower --- which, by Theorem~\ref{XII:thm:noabelian}, is a module over a
non-commutative completed Hecke algebra from the start, the object [Ch.~\ref{chap:V}, Prob.~5.1] asked for
...
\begin{remark}[Corrections to the task specification]\label{XII:rem:corrections}
(1) ``$X_\infty^{(2)}=\varprojlim(\JacH(Y_k)\otimes\Zp)$'' over an abelian tower does not
exist: there are no abelian towers (Theorem~\ref{XII:thm:noabelian}); over the finite chains the
...
ratio is a product of twisted $L$-values (Theorem~\ref{XII:thm:norm}(iii)), not a pure power of
$q$ (its $2$-adic valuations along our chains are $11,7,0,16,2$); a pure-power recursion is
what the \emph{digraph} towers obey (Theorem~\ref{XII:thm:towers}), for the ordinary sandpile
groups, not for $\JacH$. (3) The law $\mu_2p^{2k}+\mu_1p^{k}-\chi_2k+\nu_0$ splits into two
separate one-term laws, one per tower; $\chi_2=2$ is $\ord_p(q^{2})$
(Remark~\ref{XII:rem:chi2}). (4) The ``2D line complex'' $\mathcal L_2(Y_0)$ is a digraph, not a
